      ...
      const transformedRooms: RoomResponse = {
        ...
        data: propertyData?.availableRooms?.rooms?.length
          ? propertyData.availableRooms.rooms.map((room: any) => ({
              ...
              baseAmount: room.roomPrice?.[0]?.totalAmount || 0, 
              ...
              room_price: room.roomPrice?.[0]?.totalAmount || 0,
              currency_code: room.roomPrice?.[0]?.currencyCode || "USD",
              ratePlans: room.roomPrice || [],
              image: room.images,
              video: room.video,
              amenities: room.amenities,
              available_rooms: room.availabilityCount,
              rate_plan_code: room.roomPrice?.[0]?.ratePlanCode || "",
              has_valid_rate: room.hasValidRate ?? (room.roomPrice?.length > 0),
            }))
          : [],
      };
