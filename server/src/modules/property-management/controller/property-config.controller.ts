import { errorResponse, CustomRequest } from '../../../common/utils';
...
export class PropertyConfigController {
    ...
    public async updatePropertyConfig(
        ...
        try {
            ...
            if (!data) {
                return res
                    .status(400)
                    .json(errorResponse('Updated Data Payload is not there'));
            }
            const serRes =
                await this.propertyConfigService.updatePropertyConfigService(
                    propertyId,
                    data
                );
    ...
    public async getPropertyConfig(
        ...
        try {
            ...
            const serRes =
                await this.propertyConfigService.getPropertyConfig(propertyId);
            return res.status(serRes.success ? 200 : 400).json(serRes);
        } catch (error) {
            if (error instanceof Error) {
                return res
