...
import { ReservationService } from '../../../extranet/reservation/services';
...
export class AgentBookingController {
    ...
    public async createAgentBooking(
        req: AgentRequest,
        res: Response
    ): Promise<Response> {
        try {
            ...
            if (!bookingDetails.email) {
                return res
                    .status(400)
                    .json(errorResponse('Guest email is required'));
            }
            ...
            if (!bookingDetails.finalPrice) {
                return res
                    .status(400)
                    .json(errorResponse('Pricing details are required'));
            }
            ...
            if (!primaryGuest.firstName || !primaryGuest.lastName) {
                return res
                    ...
                    .json(
                        errorResponse(
                            'Primary guest first and last name are required'
                        )
                    );
            }
            ...
            if (error instanceof Error) {
                return res
                    .status(500)
                    .json(
                        errorResponse('Failed to create booking', error.message)
                    );
            }
            return res
                .status(500)
