...
import { IPaginatedResponse, nowUTC, toUTC } from '../../../../common/utils';
...
export class RatePlanRepository {
    ...
    public static async getMappedRatePlanByProperty(
        ...
        const startDateString = startDate ? toUTC(startDate) : nowUTC();
    ...
    public static async updateOrCreateChargesForDateRange(
        ...
        try {
            ...
            if (!room) {
                throw new Error(
                    `Room type with code ${roomTypeCode} not found`
                );
            }
