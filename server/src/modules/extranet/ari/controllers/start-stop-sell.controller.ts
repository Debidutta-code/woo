...
import { PropertyCustomRequest } from '../../../../common/utils';
    ...
    public async createStartStopSell(
        ...
        try {
            ...
            if (!startStopSellData) {
                return res
                    .status(400)
                    .json({ message: 'Start-stop-sell data is required' });
            }
            const response =
                await this.startStopSellService.createStartStopSell(
                    propertyId,
                    {
                        ...startStopSellData,
                        from: new Date(startStopSellData.from),
                        to: new Date(startStopSellData.to),
                    }
                );
