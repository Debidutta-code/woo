...
import { errorResponse, IApiResponse, successResponse,paginatedSuccessResponse } from '../../../../common/utils';
...
export class AgenticPropertyService {
        ...
        try {
            ...
            if (!agency) {
                return errorResponse(
                    'failed to create agentic properties',
                    'Agency not found'
                );
            }
            if (agency.isDeleted) {
            ...
            const isAlreadyExists =
                await this.agenticPropertyRepository.getAgenticPropertyByProperty(
                    data.agencyId,
                    data.propertyId
                );
            if (isAlreadyExists && isAlreadyExists.isDeleted) {
                const [result] = await Promise.all([
                    this.agenticPropertyRepository.recoverDeletedAgenticProperty(
                        isAlreadyExists.id
                    ),
                    this.agenticPropertyRepository.connectProperty(
                        data.agencyId,
                        isAlreadyExists.id
                    ),
                ]);
            ...
            if (isAlreadyExists && !isAlreadyExists.isDeleted) {
                return errorResponse(
                    'Property already connected to this agency',
                    'Agentic property already exists',
                    isAlreadyExists
                );
            }

            const agenticProperty =
                await this.agenticPropertyRepository.createAgenticProperty(
                    data
                );
            return successResponse(
                'Agentic property created successfully',
                agenticProperty
            );
        } catch (error) {
    ...
    public async deleteAgenticProperty(id: string): Promise<IApiResponse> {
        try {
            const agenticProperty =
                await this.agenticPropertyRepository.deleteAgenticProperty(id);
            if (!agenticProperty) {
                return errorResponse(
                    'failed to delete agentic property',
                    'Agentic property not found'
                );
            }
            return successResponse(
                'Agentic property deleted successfully',
                agenticProperty
            );
        } catch (error) {
            if (error instanceof Error) {
    ...
    public async getAgenticPropertyDetails(id: string): Promise<IApiResponse> {
        try {
            const agenticProperty =
                await this.agenticPropertyRepository.getAgenticPropertyById(id);
            if (!agenticProperty) {
                return errorResponse(
                    'failed to get agentic property details',
                    'Agentic property not found'
                );
            }
            return successResponse(
                'Agentic property details retrieved successfully',
                agenticProperty
            );
        } catch (error) {
            if (error instanceof Error) {
    ...
    public async getReservationsByAgents({
        ...
        try {
            const [reservations, count] = await Promise.all([
                this.agenticPropertyRepository.getReservationsByAgents(
                    agencyId,
                    propertyId,
                    skip,
                    take
                ),
                this.agenticPropertyRepository.countAllReservations(
                    agencyId,
                    propertyId
                ),
            ]);
    ...
    public async getAvailablePropertiesForAgents(
        agencyId: string
    ): Promise<IApiResponse> {
        try {
            const properties =
    ...
    public async getAgenciesByPropertyId(
        ...
        try {
            const agencies =
                await this.agenticPropertyRepository.getAgenciesByPropertyId(
                    propertyId
                );
            ...
            if (error instanceof Error) {
                return errorResponse(
                    'Failed to get agencies for property',
                    error.message
                );
            }
