import { errorResponse, IApiResponse, paginatedSuccessResponse, successResponse } from '../../../../common/utils';
...
export class LoyaltyGuestService {
    ...
    public async createGetLoyalityGuestsForProperty(
        propertyId: string,
        skip: number = 0,
        take: number = 10
    ): Promise<IApiResponse> {
    ...
    public async registerGuestFromBookingEngine(data: {
        ...
        try {
            ...
            const loyaltyConfig =
                await this.loyaltyGuestRepository.getPropertyLoyaltyConfig(
                    propertyId
                );
            ...
            const loyaltyGuest =
                await this.loyaltyGuestRepository.createGuestsLoyaltyConfigFromBookingEngine(
                    loyaltyGuestData
                );
            ...
            return successResponse(
                'Successfully registered for loyalty program',
                {
                    ...
                    currencyCode:
                        loyaltyConfig.CreationLoyaltyConfig.currencyCode,
                }
            );
        } catch (error) {
            if (error instanceof Error) {
    ...
    public async checkLoyaltyDiscount(
        ...
        try {
            ...
            return successResponse('Loyalty discount available', {
                ...
                discount: {
                    type: loyaltyConfig.CreationLoyaltyConfig
                        .loyaltyDiscountType,
