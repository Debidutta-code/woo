...
export class OccupancyBasedDynamicPricingService {
    ...
    public async createOccupancyBasedDynamicPricing(
        ...
        try {
            const [dynamicPricing, isExistsInRange, { convert, baseCurrency }] =
                await Promise.all([
                    this.dynamicPricingRepo.getDynamicPricingByPropertyIdCo(
                        propertyId
                    ),
                    this.occupancyRepo.checkIfRangeExists(
                        [],
                        data.roomId,
                        data.minInventoryPercentage,
                        data.maxInventoryPercentage
                    ),
            ...
            if (error instanceof Error) {
                return errorResponse(
                    'Error creating dynamic pricing',
                    error.message
                );
            }
            return errorResponse(
    ...
    public async getDynamicPricingByRoomId(
        roomId: string
    ): Promise<IApiResponse> {
        try {
            const dynamicPricing =
                await this.occupancyRepo.getOccupancyBasedDynamicPricingByRoomId(
                    roomId
                );
            return successResponse(
                'Occupancy-based dynamic pricing retrieved successfully ',
                dynamicPricing
            );
        } catch (error) {
            if (error instanceof Error) {
                return errorResponse(
                    'Error retrieving dynamic pricing',
                    error.message
                );
            }
            return errorResponse(
    ...
    public async updateDynamicPricing(
        id: string,
        data: ICOccupancyBasedDynamicPricing
    ): Promise<IApiResponse> {
        try {
            const [existingDynamicPricing, isExistsInRange, dynamicPricing] =
                await Promise.all([
            ...
            if (!existingDynamicPricing) {
                return errorResponse(
                    'Selected occupancy pricing not found',
                    'Selected occupancy pricing not found'
                );
            }
            ...
            if (error instanceof Error) {
                return errorResponse(
                    'Error updating dynamic pricing',
                    error.message
                );
            }
            return errorResponse(
    ...
    public async deleteDynamicPricing(id: string): Promise<IApiResponse> {
        try {
            const existingDynamicPricing =
                await this.occupancyRepo.getOccupancyBasedDynamicPricing(id);
            if (!existingDynamicPricing) {
                return errorResponse(
                    'Dynamic pricing not found',
                    'Dynamic pricing not found'
            ...
            if (error instanceof Error) {
                return errorResponse(
                    'Error deleting dynamic pricing',
                    error.message
                );
            }
            return errorResponse(
