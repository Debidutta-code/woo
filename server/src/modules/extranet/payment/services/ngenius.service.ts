...
class NGeniusService {
    ...
    async createOrder(
        ...
        try {
            ...
            if (!targetOutletId && orderData.propertyCode) {
                console.log(
                    `[N-Genius] outletId not in payload — looking up from DB for propertyCode: ${orderData.propertyCode}`
                );
                ...
                if (!property) {
                    throw new Error(
                        `[N-Genius] Property not found for code: ${orderData.propertyCode}`
                    );
                }
                ...
                if (activeIntegration?.outletId) {
                    ...
                    console.log(
                        `[N-Genius] ✅ outletId resolved from DB: ${targetOutletId}`
                    );
                } else {
                    throw new Error(
                        `[N-Genius] No active payment integration with an outletId found for property: ${property.id}`
                    );
                }
            }
            ...
            if (!targetOutletId) {
                throw new Error(
                    `[N-Genius] outletId is required but was not provided and could not be resolved.`
                );
            }
            ...
            if (orderData.propertyCode) {
                try {
                    ...
                    if (property) {
                        const ngeniusState =
                            response.data._embedded?.payment?.[0]?.state ||
                            'STARTED';
    ...
    async cancelCapture(
        ...
        try {
            ...
            console.log(
                `[CANCEL CAPTURE - Step 2] 🗂️  Captures found: ${captures?.length ?? 0}`
            );

            if (!captures || captures.length === 0) {
                console.warn(
                    `[CANCEL CAPTURE - Step 2] ⚠️ No captures found. Payment state: ${payment.state}`
                );
                return {
                    success: false,
            ...
            if (!captureSelfHref) {
                console.error(
                    `[CANCEL CAPTURE - Step 3] ❌ capture[0]._links.self.href not found`
                );
                console.error(
                    `[CANCEL CAPTURE - Step 3]    Full capture[0]._links:`,
                    JSON.stringify((captures[0] as any)._links, null, 2)
                );
                return {
                    success: false,
                    message:
                        'Capture self href not found — cannot cancel capture',
                };
            }
            ...
            console.log(
                `   HTTP Status: ${cancelCaptureResponse.status} ${cancelCaptureResponse.statusText}`
            );
            console.log(
                `   Response:`,
                JSON.stringify(cancelCaptureResponse.data, null, 2)
            );
            console.log(`${'='.repeat(60)}\n`);
            ...
            if (axios.isAxiosError(error)) {
                ...
                const errMessage =
                    ...
                    'Cancel capture API request failed';
                return { success: false, message: errMessage };
            }
            const msg =
                error instanceof Error
                    ? error.message
                    : 'Unknown error during cancel capture';
    ...
    async reverseAuthorization(
        ...
        try {
            ...
            if (!cancelHref) {
                console.error(
                    `[REVERSE AUTH - Step 2] ❌ cnp:cancel href not found`
                );
                console.error(
                    `   Full payment._links:`,
                    JSON.stringify((payment as any)._links, null, 2)
                );
                return {
                    success: false,
                    message:
                        'Authorization cancel href not found — payment may not be in AUTHORISED state',
                };
            }
            ...
            console.log(
                `   HTTP Status : ${reverseResponse.status} ${reverseResponse.statusText}`
            );
            console.log(`   State       : ${reverseResponse.data?.state}`);
            console.log(
                `   Response    :`,
                JSON.stringify(reverseResponse.data, null, 2)
            );
            console.log(`${'='.repeat(60)}\n`);
            ...
            if (axios.isAxiosError(error)) {
                ...
                const errMessage =
                    errData?.message ||
                    errData?.errors?.[0]?.message ||
                    'Authorization reversal API request failed';
                return { success: false, message: errMessage };
            }
            const msg =
                error instanceof Error
                    ? error.message
                    : 'Unknown error during authorization reversal';
            return { success: false, message: msg };
        }
    }
    ...
    async processSameDayRefund(
        ...
        try {
            ...
            const cancelResult = await this.cancelCapture(
                orderReference,
                outletId
            );
            ...
            if (!cancelResult.success) {
                console.error(
                    `[SAME-DAY REFUND] ❌ Cancel capture failed: ${cancelResult.message}`
                );
                return {
                    success: false,
            ...
            console.log(
                `[SAME-DAY REFUND] ✅ Capture cancelled. Proceeding to authorization reversal...`
            );
            ...
            const reverseResult = await this.reverseAuthorization(
                orderReference,
                outletId
            );
            ...
            if (!reverseResult.success) {
                console.error(
                    `[SAME-DAY REFUND] ❌ Authorization reversal failed: ${reverseResult.message}`
                );
                return {
                    success: false,
            ...
            console.log(
                `[SAME-DAY REFUND] ✅ Authorization reversed. Same-day refund complete!`
            );
            console.log(`${'='.repeat(60)}\n`);
            ...
            return {
                success: true,
                message:
                    'Same-day refund processed successfully (capture cancelled + authorization reversed)',
    ...
    async processRefund(
        ...
        try {
            ...
            console.log(`📋 Order Reference : ${orderReference}`);
            console.log(
                `🏪 Outlet ID       : ${outletId ?? '⚠️ (NOT PROVIDED)'}`
            );
            console.log(`${'='.repeat(60)}`);
            ...
            if (!captures || captures.length === 0) {
                console.error(
                    `[REFUND] ❌ No captures found. Payment state: ${payment.state}`
                );
                return {
                    success: false,
                    message:
                        'No capture found for this payment (payment may not be in CAPTURED state)',
                };
            }
            ...
            if (!rawCaptureHref) {
                return {
                    success: false,
                    message: 'Capture refund href not found in order status',
                };
            }
            ...
            if (!rawCaptureHref.endsWith('/refund')) {
                return {
                    success: false,
                    message:
                        'Reservation cannot be cancelled within 24 hours of booking. Please try again after 24 hours.',
                };
            }
            ...
            const captureRef =
                capturesIndex !== -1
                    ? refundUrlParts[capturesIndex + 1]
                    : 'unknown';
