...
class WebhookService {
    ...
    async processWebhookEvent(payload: NGeniusWebhookPayload): Promise<void> {
        ...
        if (
            payload.order._embedded?.payment &&
            payload.order._embedded.payment.length > 0
        ) {
        ...
        console.log(
            `🔑 Redis key being SET: payment:confirmed:${orderReference}`
        );
        ...
        if (status === 'success' || status === 'failed') {
            ...
            try {
                ...
                console.log(
                    `✅ Payment result (${status}) stored in Redis for ${orderReference}`
                );

                const verify = await client.get(redisKey);
                console.log(
                    `🔍 Redis verify read-back: ${verify ? 'KEY EXISTS ✅' : 'KEY MISSING ❌ - write failed silently'}`
                );
            } catch (err: any) {
                console.error(
                    `❌ Failed to store payment result in Redis for ${orderReference}`,
                    err
                );
            }
        }
     ...
     */
    private async updatePaymentDatabase(
