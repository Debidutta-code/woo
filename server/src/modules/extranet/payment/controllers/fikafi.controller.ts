...
import { errorResponse, PropertyRequest, successResponse } from '../../../../common/utils';
...
export class FikafiPaymentController {
    ...
    public static async handlePaymentEventWebhook(req: Request, res: Response) {
        try {
            ...
            if (isPaid) {
                ...
                console.log(
                    `🔍 Redis verify read-back: ${isExists ? 'KEY EXISTS ✅' : 'KEY MISSING ❌ - write failed silently'}`
                );
                ...
                console.log(
                    `❌ Payment failed for ${bookingRefNum} (status: ${status})`
                );
                ...
                try {
                    ...
                    console.log(
                        `✅ Reservation ${bookingRefNum} marked as ${bookingStatus}`
                    );
                } catch (dbError) {
                    console.error(
                ...
                try {
                    ...
                    console.log(
                        `✅ Payment failure stored in Redis for ${bookingRefNum}`
                    );
                } catch (redisError) {
    ...
    public static async takePaymentAction(req: Request, res: Response) {
        try {
            ...
            console.log(
                `📤 Taking payment action: ${action} for booking ${bookingRefNum}, fikafi ref: ${fikafiRefNum}`
            );
            ...
            if (result.success) {
                ...
                if (action === 'cancel') {
                    try {
                        ...
                        console.log(
                            `✅ Reservation ${bookingRefNum} marked as cancelled`
                        );
                    } catch (dbError) {
                        console.error(
                            `❌ Failed to update reservation in DB:`,
