...
export class NGeniusController {
    ...
    static async createOrder(
        ...
        try {
            ...
            console.log(
                '📥 [BACKEND DEBUG] Received N-Genius order payload:',
            ...
            if (!orderData.amount.currencyCode || !orderData.amount.value) {
                res.status(400).json({
                    success: false,
                    message:
                        'Invalid amount. currencyCode and value are required.',
                });
                return;
            }
    ...
    static async getOrderStatus(
        ...
        try {
            ...
            const orderStatus =
                await ngeniusService.getOrderStatus(orderReference);
    ...
    static async processRefund(
        ...
        try {
            ...
            const refundStrategy: 'same_day' | 'day_after' =
                (req as any).refundStrategy ?? 'day_after';
            const resolvedOutletId: string | undefined = (req as any)
                .resolvedOutletId;

            console.log(`\n[REFUND CONTROLLER] 💡 Strategy: ${refundStrategy}`);
            console.log(
                `[REFUND CONTROLLER] 📋 Order Reference: ${orderReference}`
            );
            console.log(
                `[REFUND CONTROLLER] 🏪 Outlet ID: ${resolvedOutletId ?? '(not resolved)'}`
            );
            ...
            if (refundStrategy === 'same_day') {
                if (!resolvedOutletId) {
                    res.status(400).json({
                        success: false,
                        message:
                            'outletId is required for same-day refund but could not be resolved from the database.',
                    });
                    return;
                }

                console.log(
                    `[REFUND CONTROLLER] ⚡ Routing to SAME-DAY refund (cancel capture + reverse auth)`
                );
                refundResult = await ngeniusService.processSameDayRefund(
                    orderReference,
                    resolvedOutletId
                );
            } else {
                console.log(
                    `[REFUND CONTROLLER] 🕐 Routing to DAY-AFTER refund (standard refund API)`
                );
