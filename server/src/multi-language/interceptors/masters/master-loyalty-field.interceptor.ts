...
export class MasterLoyaltyFieldInterceptor {
    public static async intercept(
        ...
        try {
            ...
            console.error(`[MasterLoyaltyFieldInterceptor Error]:`, error);
            return response;
        }
    }
    ...
    private static async attachTranslation(item: any, locale: string): Promise<any> {
        ...
        const translation = await MasterLoyaltyRegistrationFieldTranslation.getTranslated(item.id, locale);
        if (translation) {
