...
export class MasterAmenityInterceptor {
    public static async intercept(
        ...
        try {
            ...
            console.error(`[MasterAmenityInterceptor Error]:`, error);
            return response;
        }
    }
    ...
    private static async attachTranslation(item: any, locale: string): Promise<any> {
        ...
        const translation = await MasterAmenityTranslation.getTranslated(item.id, locale);
        if (translation) {
