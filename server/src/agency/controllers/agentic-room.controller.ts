...
export class AgenticRoomController {
    ...
    public async createRoomsForAgent(
        req: CustomRequest,
        res: Response
    ): Promise<Response> {
        try {
            const { agenticPropertyId, roomId, roomType, roomName, isActive } =
                req.body;
            ...
            if (error instanceof Error) {
                return res
                    .status(500)
                    .json(
                        errorResponse('failed to create room', error.message)
                    );
            }
            return res
                .status(500)
    ...
    public async getRoomsForAgencies(
        ...
        try {
            ...
            if (!agenticPropertyId) {
                return res
                    .status(400)
                    .json(errorResponse('agenticPropertyId is required'));
            }
            if (!propertyId) {
                return res
                    .status(400)
        ...
        try {
            ...
            if (!agenticPropertyId || !agenticRoomId) {
                return res
                    ...
                    .json(
                        errorResponse(
                            'agenticPropertyId and agenticRoomId are required'
                        )
                    );
            }

            const result = await this.agenticRoomService.removeAgenticRoom(
                agenticPropertyId,
                agenticRoomId
            );
            ...
            if (error instanceof Error) {
                return res
                    .status(500)
                    .json(
                        errorResponse('failed to remove room', error.message)
                    );
            }
            return res
                .status(500)
    ...
    public async updateAgenticRoomAvailability(
        ...
        try {
            ...
            const result =
                await this.agenticRoomService.updateAgenticRoomAvailability(
                    id,
                    isAvailable
                );
            return res.status(result.success ? 200 : 404).json(result);
    ...
    public async addRoomsToAgencies(
        ...
        try {
            ...
            if (!agenticPropertyId) {
                return res
                    .status(400)
                    .json(errorResponse('Agents property ID is not chosen'));
            }
            if (roomIds.length == 0 || !roomIds) {
                return res
                    .status(400)
                    .json(errorResponse('Rooms are not selected are required'));
            }
            const result = await this.agenticRoomService.addRoomsToAgencies(
                agenticPropertyId,
                roomIds
            );
            return res.status(result.success ? 200 : 404).json(result);
