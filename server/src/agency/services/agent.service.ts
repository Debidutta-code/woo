...
export class AgentService {
    ...
    public async createAgents(data: ICAgents): Promise<IApiResponse> {
        try {
            ...
            if (isAlreadyExists) {
                return errorResponse("Agent with the same email already exists", "agent exists with same email");
            }
    ...
    public async getAgents(agencyId: string, page: number, limit: number): Promise<IApiResponse> {
        try {
