...
import { ActivityAction, ActivityEntity } from '../../modules/logs/model/activity.model';
import { IActivityConfig } from './activityLogger';
import { AgentRequest } from '../../modules/agent-paltform/utils';
...
export const createCRUDConfig = (
    ...
    if (!options?.skipCreate) {
        configs.push({
            ...
            getEntityName: (req, resBody) =>
                resBody?.data?.[entityNameField] || req.body?.[entityNameField],
    ...
    if (!options?.skipUpdate) {
        configs.push({
            ...
            getEntityId: req =>
                req.params?.id ||
                req.params?.[`${entity.toLowerCase()}Id`] ||
                'unknown',
            getEntityName: (req, resBody) =>
                resBody?.data?.[entityNameField] || req.body?.[entityNameField],
            getDescription: (req, resBody, statusCode) => {
...
export const createSimpleConfig = (
    ...
    entity,
    getEntityId: (req, resBody) =>
        resBody?.data?.id || req.params?.id || 'unknown',
    getDescription: (req, resBody, statusCode) => {
        const isSuccess = statusCode! >= 200 && statusCode! < 300;
...
export const logOnlySuccess: IActivityConfig['shouldLog'] = (
    req,
    resBody,
    statusCode
) => {
...
export const logOnlyFailures: IActivityConfig['shouldLog'] = (
    req,
    resBody,
    statusCode
) => {
...
export const logCriticalOnly = (
    criticalActions: ActivityAction[]
): IActivityConfig['shouldLog'] => {
    return (req, resBody, statusCode) => {
        return statusCode !== undefined;
    };
};

export const createCheckInOutConfig = (
    action: ActivityAction.CHECKIN | ActivityAction.CHECKOUT
): IActivityConfig => ({
    action,
    ...
    getDescription: (req, resBody, statusCode) => {
        const isSuccess = statusCode! >= 200 && statusCode! < 300;
        const actionText =
            action === ActivityAction.CHECKIN ? 'checked in' : 'checked out';
        return isSuccess
...
export const createPaymentConfig = (
    ...
    tags: [
        'payment',
        entity === ActivityEntity.PAYMENT ? 'transaction' : 'refund',
    ],
});
/**
...
export const createPromotionConfig = (
    ...
    return [
        // CREATE
        {
            ...
            getEntityName: (req, resBody) =>
                resBody?.data?.[entityNameField] ||
                req.body?.[entityNameField] ||
                'unknown',
            getDescription: (req, resBody, statusCode) => {
                const isSuccess = statusCode! >= 200 && statusCode! < 300;
                const name =
        ...
        // DELETE
        {
            ...
            getEntityName: (req, resBody) =>
                resBody?.data?.[entityNameField] || 'unknown',
            getDescription: (req, resBody, statusCode) => {
                const isSuccess = statusCode! >= 200 && statusCode! < 300;
                const name =
                    resBody?.data?.[entityNameField] || `deleted ${entityName}`;
                return isSuccess
...
export const createToggleStatusConfig = (
    ...
    getDescription: (req, resBody, statusCode) => {
        const isSuccess = statusCode! >= 200 && statusCode! < 300;
        const status =
            resBody?.data?.isActive || resBody?.data?.status || 'toggled';
        return isSuccess
...
export const createLoyaltyConfig = (
    ...
    return [
        ...
        // DELETE
        {
            ...
            getEntityId: (req, resBody) => req.params?.id || 'unknown',
            getEntityName: (req, resBody) =>
                resBody?.data?.[nameField] || 'unknown',
            getDescription: (req, resBody, statusCode) => {
                const isSuccess = statusCode! >= 200 && statusCode! < 300;
                return isSuccess
...
export const createPropertySubResourceConfig = (
    ...
    getEntityId: (req, resBody) =>
        req.params?.propertyId || req.params?.id || 'unknown',
    getEntityName: (req, resBody) =>
        resBody?.data?.propertyName || req.body?.propertyName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
        const isSuccess = statusCode! >= 200 && statusCode! < 300;
        const propertyName =
            resBody?.data?.propertyName || req.body?.propertyName || 'property';
        const actionText =
            action === ActivityAction.CREATE
                ? 'added to'
                : action === ActivityAction.UPDATE
...
export const createRoomSubResourceConfig = (
    ...
    getEntityId: (req, resBody) =>
        req.params?.roomId || req.params?.id || 'unknown',
    getEntityName: (req, resBody) =>
...
export const createTaxConfig = (
    ...
    const entity = taxType.includes('group')
        ? ActivityEntity.TAX_GROUP
        : ActivityEntity.TAX_RULE;
    return [
        // CREATE
        {
            action: ActivityAction.CREATE,
...
export const createManagementConfig = (
    ...
    getEntityId: (req, resBody) =>
        resBody?.data?.id || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) =>
        resBody?.data?.[nameField] || req.body?.[nameField] || 'unknown',
    getDescription: (req, resBody, statusCode) => {
        const isSuccess = statusCode! >= 200 && statusCode! < 300;
        const name =
            resBody?.data?.[nameField] || req.body?.[nameField] || resourceType;
        const actionText =
            action === ActivityAction.CREATE
                ? 'created'
                : action === ActivityAction.UPDATE
...
export const createVideoConfig = (
    ...
    getDescription: (req, resBody, statusCode) => {
        ...
        const name =
            ...
            targetType;
        const actionText =
            action === ActivityAction.CREATE ? 'uploaded' : 'deleted';
        return isSuccess
