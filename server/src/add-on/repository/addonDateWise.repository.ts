...
export class AddonDateWiseDao {
     ...
     */
    public async upsertAddonDateWise(
        ...
        try {
            ...
            throw new Error(
                error?.message ||
                    'Failed to upsert addon date-wise availability'
            );
        }
    }
    ...
    public async upsertAddonDateWiseWithMetadata(
        ...
        try {
            ...
            const existingKeys = new Set(
                existingRecords.map(
                    (r: any) => `${r.addonId}_${r.date.toISOString()}`
                )
            );
    ...
    public static async getAvailableAddonsByDateRange(
        ...
        try {
            ...
            throw new Error(
                `Error fetching available addons for date range: ${error?.message}`
            );
        }
    }
}
