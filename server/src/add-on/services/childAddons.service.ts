import { ChildAddonRepository } from '../repository';
...
export class ChildAddonsService {
    ...
    public async createChildAddon(
        data: ICChildAddoon,
        propertyId: string
    ): Promise<IApiResponse> {
        try {
            const [getAllChildAddons, { convert, baseCurrency }] =
                await Promise.all([
            ...
            if (!isAgeValid) {
                return errorResponse(
                    'Some children catalog are overlapping in age range'
                );
            }
            ...
            const daoRes = await this.childAddonRepository.createChildAddons({
                ...
                discountAmount:
                    data.discountType === 'flat'
                        ? convert(data.discountAmount ? data.discountAmount : 0)
                        : data.discountAmount,
                currencyCode:
                    data.discountType === 'flat'
                        ? baseCurrency
            ...
            if (daoRes) {
                return successResponse(
                    'Children catalog created successfully',
                    daoRes
                );
            }
    ...
    private validateAge(
        ...
        return !existingChildAddons.some(
            addon =>
    ...
    public async getChildAddons(addonId: string): Promise<IApiResponse> {
        try {
            const childAddons =
                await this.childAddonRepository.getChildAddons(addonId);
            return successResponse(
                'Child addons retrieved successfully',
                childAddons
            );
        } catch (error) {
            if (error instanceof Error) {
                return errorResponse(
                    'Failed to retrieve child addons',
                    error.message
                );
            }
    ...
    public async updateChildAddon(
        ...
        try {
            ...
            const daoRes = await this.childAddonRepository.updateChildAddon(
                id,
                {
                    minAge: data.minAge,
                    maxAge: data.maxAge,
            ...
            if (daoRes) {
                return successResponse(
                    'Children catalog updated successfully',
                    daoRes
                );
            }
