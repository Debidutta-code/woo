...
export class BookingAddonService {
    ...
    async createBookingAddon(
        bookingAddonData: IBookingAddon
    ): Promise<IApiResponse> {
        try {
            // Check if addon exists
            const addon = await AddonRepository.getAddonById(
                bookingAddonData.addonId.toString()
            );
            if (!addon) {
            ...
            const calculatedTotal =
                bookingAddonData.unitPrice * bookingAddonData.quantity;
            if (
                Math.abs(calculatedTotal - bookingAddonData.totalPrice) > 0.01
            ) {
                return errorResponse(
                    'Total price does not match unit price × quantity',
                    'Validation Error'
                );
            }

            const bookingAddon =
                await BookingAddOnRepository.createBookingAddon(
            ...
            return successResponse(
                'Booking addon created successfully',
                bookingAddon
            );
        } catch (error: any) {
            ...
            console.error(
                'Failed to create booking addon at Service Layer:',
                error
            );
            return errorResponse(
                'Failed to create booking addon',
    ...
    async updateBookingAddon(
        ...
        try {
            ...
            if (
                ...
                const calculatedTotal =
                    updateData.unitPrice * updateData.quantity;
            ...
            const updatedBookingAddon =
                await BookingAddOnRepository.updateBookingAddOnByBookingId(
                    bookingAddonId,
                    updateData
                );
            ...
            return successResponse(
                'Booking addon updated successfully',
                updatedBookingAddon
            );
        } catch (error: any) {
            if (error instanceof Error) {
            ...
            console.error(
                'Failed to update booking addon at Service Layer:',
                error
            );
            return errorResponse(
                'Failed to update booking addon',
    ...
    async deleteBookingAddon(bookingAddonId: string): Promise<IApiResponse> {
        try {
            const result =
                await BookingAddOnRepository.deleteBookingAddOnById(
                    bookingAddonId
                );
            ...
            return successResponse(
                'Booking addon deleted successfully',
                result
            );
        } catch (error: any) {
            if (error instanceof Error) {
            ...
            console.error(
                'Failed to delete booking addon at Service Layer:',
                error
            );
            return errorResponse(
                'Failed to delete booking addon',
