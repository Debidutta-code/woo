...
import { RoomBookingService } from '../../../modules/extranet/reservation-pricing/service';

export class GoogleFeedsService {
    /**
     * Generate Hotel List Feed Data
     */
    public static async generateHotelListFeed(): Promise<
        IGoogleHotelListItem[]
    > {
        ...
        for (let i = 0; i < properties.length; i++) {
            ...
            try {
                ...
                const propertyPrices =
                    await this.generateLowestPricesForProperty(
                ...
                console.error(
                    `    ❌ Error processing ${property.propertyCode}:`,
                    error
                );
            }
        }
     ...
     */
    public static async generateLandingPageFeed(): Promise<
        IGoogleLandingPageItem[]
    > {
        const properties = await GoogleFeedsRepository.getAllActiveProperties();
    ...
    private static async generateLowestPricesForProperty(
        ...
        for (
            let dayOffset = 0;
            dayOffset < GOOGLE_FEED_CONFIG.DAYS_AHEAD;
            dayOffset++
        ) {
            ...
            checkOutDate.setDate(
                checkInDate.getDate() + GOOGLE_FEED_CONFIG.DEFAULT_NIGHTS
            );
        ...
        for (let i = 0; i < datePairs.length; i += concurrency) {
                ...
                try {
                    ...
                    if (response.success && response.data?.rooms) {
                        for (const room of response.data.rooms) {
                            if (!room.hasValidRate || !room.roomPrice.length)
                                continue;
                            ...
                            for (const rateplan of room.roomPrice) {
                                ...
                                if (
                                    !lowestPrice ||
                                    currentPrice.baseRate < lowestPrice.baseRate
                                ) {
                    ...
                    console.error(
                        `    ⚠️ Error fetching ${propertyCode} for ${checkIn}:`,
                        error
                    );
                    return null;
                }
            ...
            const validPrices = batchResults.filter(
                (price): price is IGooglePriceItem => price !== null
            );
