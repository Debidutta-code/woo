...
`;
    ...
    for (const [propertyCode, propertyPrices] of Object.entries(groupedByProperty)) {
      xml += `  <PropertyDataSet>
...
`;
   ...
   */
  public static buildLandingPageFeed(landingPages: IGoogleLandingPageItem[]): string {
    const timestamp = new Date().toISOString();
    const transactionId = Date.now();
...
`;
   ...
   */
  private static groupByProperty(prices: IGooglePriceItem[]): Record<string, IGooglePriceItem[]> {
