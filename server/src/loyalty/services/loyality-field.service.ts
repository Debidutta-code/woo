...
import { LoyalityFormFieldRepository } from '../repository';
import {
    ICLoyaltyField,
    ILoyaltyField,
    IULoyaltyField,
...
export class LoyalityFieldService {
    ...
    public async createFields({
        ...
        required,
    }: ICLoyaltyField): Promise<IApiResponse> {
        try {
            const isAlreadyExists =
                await this.loyalityFieldRepository.checkIfFieldExists(
                    loyaltyProgramId,
                    fieldName
                );
            if (isAlreadyExists) {
                return errorResponse(
                    'Field already exists in this loyalty program'
                );
            }
            const daoRes =
                await this.loyalityFieldRepository.createLoyalityFormField({
                    loyaltyProgramId,
                    masterRegistrationFieldId,
                    fieldName,
                    visibleInRegistration,
    ...
    public async updateLoyalityFields(
        ...
        try {
            ...
            if (!isFieldExists) {
                return errorResponse(
                    'Field does not exist in this loyalty program'
                );
            }
            const daoRes =
                await this.loyalityFieldRepository.updateLoyalityFormField(
                    loyaltyProgramId,
                    fieldName,
                    data
                );
            ...
            if (error instanceof Error) {
                return errorResponse(
                    'Failed to update fields in forms',
                    error.message
                );
            }
            return errorResponse('Failed to update fields in forms');
    ...
    public async deleteLoyalityFields(
        loyaltyProgramId: string,
        fieldName: string
    ): Promise<IApiResponse> {
        try {
            ...
            const daoRes =
                await this.loyalityFieldRepository.deleteLoyalityFormField(
                    loyaltyProgramId,
                    fieldName
                );
            if (!daoRes) {
            ...
            if (error instanceof Error) {
                return errorResponse(
                    'Failed to delete fields in forms',
                    error.message
                );
            }
            return errorResponse('Failed to delete fields in forms');
    ...
    public async updateManyFieldsService(
        loyaltyProgramId: string,
        ...
        try {
            ...
            for (const field of fields) {
                if (!field.fieldName) {
                    return errorResponse(
                        'Field name is required for all fields'
                    );
                }
            }

            const results = await this.loyalityFieldRepository.updateManyFields(
                loyaltyProgramId,
                fields
            );
