...
export default class AccessControl {
    public static async createNewRole(req: CustomRequest, res: Response) {
        try {
            ...
            if (data.level < 0 || data.level > 4) {
                return res
                    .status(400)
                    .json(errorResponse('Hierarchy not defined'));
            }
            if (!data.role || data.role == '') {
                return res
                    .status(400)
                    .json(errorResponse('Role must be defined'));
            }
    ...
    public static async getAccessForRole(req: CustomRequest, res: Response) {
        try {
            const role = req.params.role.toString() as
                | 'staff'
                | 'super_admin'
                | 'group_manager'
                | 'hotel_manager'
    ...
    public static async updateAccessForLevel(
        req: CustomRequest,
        res: Response
    ) {
        try {
            ...
            const { data } = req.body;
            const serviceRes = await AccessService.updateAccessForLevel(
                role,
                data
            );
            if (serviceRes.success) {
    ...
    public static async deleteRoleController(
        req: CustomRequest,
        res: Response
    ) {
        try {
