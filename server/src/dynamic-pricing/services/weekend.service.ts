...
import {
    DynamicPricing,
    WeekendDynamicPricingRepository
} from "../repository"
...
export class WeekendDynamicPricingService {
    ...
    public async createWeekendDynamicPricing(propertyId: string, data: ICWeekendDynamicPricingS): Promise<IApiResponse> {
        try {
            const [dynamicPricing, { convert, baseCurrency }, isExistsInDateRange] = await Promise.all([
                ...
                this.weekendRepo.weekendDynamicPricingByDateRange([], data.roomId, data.startDate, data.endDate)

            ]);
            if (isExistsInDateRange && isExistsInDateRange.length > 0) {
                return errorResponse("Weekend dynamic pricing already exists for this date range", "Weekend dynamic pricing already exists");
            }
            if (!dynamicPricing) {
            ...
            return successResponse("Weekend dynamic pricing created successfully");
        } catch (error) {
            if (error instanceof Error) {
                return errorResponse("Error creating weekend dynamic pricing", error.message);
            }
            return errorResponse("Error creating weekend dynamic pricing", "Unknown error");
        }
    }
    public async getByRoomId(roomId: string): Promise<IApiResponse> {
        try {
            ...
            if (error instanceof Error) {
                return errorResponse("Error fetching weekend dynamic pricing", error.message);
            }
            return errorResponse("Error fetching weekend dynamic pricing", "Unknown error");
        }
    }
    public async updateWeekendDynamicPricing(id: string, data: ICWeekendDynamicPricing): Promise<IApiResponse> {
        try {
            const [updatedWeekendDynamicPricing, dynamicPricing, isExistsInDateRange] = await Promise.all([
                this.weekendRepo.updateWeekendDynamicPricing(id, data),
                this.dynamicPricingRepo.getById(data.dynamicId),
                this.weekendRepo.weekendDynamicPricingByDateRange([id], data.roomId, data.startDate, data.endDate)  
            ]);
            if (!dynamicPricing) {
                return errorResponse("Dynamic pricing not found", "Dynamic pricing not found");
            }
            ...
            return errorResponse("Error updating weekend dynamic pricing", "Unknown error");
        }
    }
    public async deleteWeekendDynamicPricing(id: string): Promise<IApiResponse> {
        try {
            const existingWeekendPricing = await this.weekendRepo.getWeekendDynamicPricing(id);
            if (!existingWeekendPricing) {
                return errorResponse("Weekend dynamic pricing not found", "Weekend dynamic pricing not found");
            }
            await this.weekendRepo.deleteWeekendDynamicPricing(id);
            return successResponse("Weekend dynamic pricing deleted successfully");
        } catch (error) {
            if (error instanceof Error) {
                return errorResponse("Error deleting weekend dynamic pricing", error.message);
            }
            return errorResponse("Error deleting weekend dynamic pricing", "Unknown error");
        }
    }
}