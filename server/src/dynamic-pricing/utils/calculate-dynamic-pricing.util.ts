...
export class DynamicPricingCalculator {
    public async calculateDynamicPricingForDateRange(
        ...
        currentAvailableRoom: number | null = null
    ): Promise<IDynamicPricingResult[]> {
        const results: IDynamicPricingResult[] = [];
        const currentDate = new Date(startDate);

        while (currentDate < endDate) {
            const availableRooms = currentAvailableRoom !== null
                ? currentAvailableRoom
                : await this.getAvailableRooms(propertyId, roomTypeCode, currentDate);
    ...
    private async getAvailableRooms(propertyId: string, roomTypeCode: string, date: Date): Promise<number | null> {
        try {
            ...
            return availableRooms?.availability ?? null;
        } catch (error) {
            throw new Error(
                `Failed to get available rooms for room ${roomTypeCode} on ${date}`
            );
        }
    }
