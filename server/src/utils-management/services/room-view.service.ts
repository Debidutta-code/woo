...
export class MasterRoomViewService {
    ...
    ): Promise<IApiResponse<IMasterRoomView>> {
        try {
            const existingView =
                await this.masterRoomViewRepository.getRoomViewByName(
                    data.viewName
                );
            if (existingView) {
            ...
            const roomView =
                await this.masterRoomViewRepository.createRoomView(data);
            if (roomView) {
    ...
    public async getAllRoomViews(): Promise<IApiResponse<IMasterRoomView[]>> {
        try {
            const roomViews =
                await this.masterRoomViewRepository.getAllRoomViews();
    ...
    public async getRoomViewById(
        id: string
    ): Promise<IApiResponse<IMasterRoomView>> {
        try {
            const roomView =
                await this.masterRoomViewRepository.getRoomViewById(id);
            if (roomView) {
                return successResponse(
                    'RoomView fetched successfully',
                    roomView
                );
            }
    ...
    public async updateRoomView(
        ...
        try {
            ...
            const roomView = await this.masterRoomViewRepository.updateRoomView(
                id,
                data
            );
            if (roomView) {
                return successResponse(
                    'RoomView updated successfully',
                    roomView
                );
            }
            return errorResponse('RoomView not found');
    ...
    public async deleteRoomView(
        ...
        try {
            ...
            const roomView =
                await this.masterRoomViewRepository.deleteRoomView(id);
            if (roomView) {
                return successResponse(
                    'RoomView deleted successfully',
                    roomView
                );
            }
            return errorResponse('RoomView not found');
