...
export class AminityServices {
  ...
  public  async createCategory(amenities: string[]) :Promise<IApiResponse> {
    try {
      const daoRes = await this.propertyAminityDao.addPropertyAmenities(amenities);
  ...
  public  async getCategory(type: string = "property") :Promise<IApiResponse> {
    try {
      const daoRes = await this.propertyAminityDao.getAllPropertyAmenities(type);
  ...
  public  async deleteCategory(amenities: string[]) :Promise<IApiResponse> {
    try {
      const daoRes = await this.propertyAminityDao.deletePropertyAmenities(amenities);
