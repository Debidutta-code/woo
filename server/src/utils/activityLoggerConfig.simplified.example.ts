...
export const SIMPLIFIED_ACTIVITY_LOGGER_ROUTES: RoutePattern[] = [
    ...
    // TAX_RULE
    {
        ...
        config: {
            ...
            getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name,
            getDescription: (req, resBody, statusCode) => {
                const isSuccess = statusCode! >= 200 && statusCode! < 300;
                return isSuccess
...

    {
        ...
        config: {
            ...
            getEntityId: (req, resBody) => resBody?.data?.id || 'unknown',
            getEntityName: (req, resBody) => resBody?.data?.email || req.body?.email,
            getDescription: (req, resBody, statusCode) => {
                const isSuccess = statusCode! >= 200 && statusCode! < 300;
                return isSuccess
    ...
    // ACCESS CONTROL
    {
        ...
        config: {
            ...
            getEntityId: (req, resBody) => resBody?.data?.roleId || resBody?.data?._id || 'unknown',
            getEntityName: (req, resBody) => resBody?.data?.roleName || req.body?.roleName || req.body?.role || 'unknown',
            getDescription: (req, resBody, statusCode) => {
                const isSuccess = statusCode! >= 200 && statusCode! < 300;
                const roleName = resBody?.data?.roleName || req.body?.roleName || req.body?.role || 'unknown';
                return isSuccess
    ...
    },
    {
        ...
        config: {
            action: ActivityAction.UPDATE,
            entity: ActivityEntity.ACCESS_CONTROL,
            getEntityId: (req, resBody) => req.params?.role || resBody?.data?.roleId || 'unknown',
            getEntityName: (req, resBody) => req.params?.role || resBody?.data?.roleName || 'unknown',
            getDescription: (req, resBody, statusCode) => {
                const isSuccess = statusCode! >= 200 && statusCode! < 300;
                const roleName = req.params?.role || 'unknown';
                const permissionsCount = req.body?.permissions ? Object.keys(req.body.permissions).length : 0;
                return isSuccess
            ...
            tags: ['access-control', 'role-management', 'update', 'permissions', 'security']
        }
    },
    {
    ...
    config: {
      ...
      getEntityName: (req, resBody) => resBody?.data?.addonName || req.body?.addonName || 'unknown',
      getDescription: (req, resBody, statusCode) => {
        const isSuccess = statusCode! >= 200 && statusCode! < 300;
        const addonName = resBody?.data?.addonName || req.body?.addonName || 'unknown';
  ...
  // Update Addon by Addon ID
  {
    ...
    config: {
      action: ActivityAction.UPDATE,
      entity: ActivityEntity.ADDON_AVAILABILITY,
      getEntityId: (req, resBody) => req.params?.addonId || resBody?.data?._id || 'unknown',
      getEntityName: (req, resBody) => resBody?.data?.addonName || req.body?.addonName || 'unknown',
      getDescription: (req, resBody, statusCode) => {
        const isSuccess = statusCode! >= 200 && statusCode! < 300;
        const addonName = resBody?.data?.addonName || req.body?.addonName || 'addon';
        return isSuccess
  ...
  // Update Addon for Single Date
  {
    ...
    config: {
      ...
      getDescription: (req, resBody, statusCode) => {
        const isSuccess = statusCode! >= 200 && statusCode! < 300;
        const date = resBody?.data?.date || req.body?.date || 'specific date';
        return isSuccess
  ...
  // Delete Addon by Addon ID
  {
    ...
    config: {
      ...
      getEntityName: (req, resBody) => resBody?.data?.addonName || 'unknown',
      getDescription: (req, resBody, statusCode) => {
        const isSuccess = statusCode! >= 200 && statusCode! < 300;
  ...
  // Agent Login
{
  ...
  config: {
    ...
    getEntityId: (req, resBody) => resBody?.data?.agentId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.email || req.body?.email || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const email = req.body?.email || 'unknown';
      return isSuccess
...
// Create Agent
{
  ...
  config: {
    ...
    entity: ActivityEntity.AGENT,
    getEntityId: (req, resBody) => resBody?.data?.agentId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.email || req.body?.email || resBody?.data?.name || req.body?.name || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const agentName = resBody?.data?.name || req.body?.name || resBody?.data?.email || req.body?.email || 'unknown';
      return isSuccess
...
// Delete Agent
{
  ...
  config: {
    ...
    entity: ActivityEntity.AGENT,
    getEntityId: (req, resBody) => req.params?.id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.email || resBody?.data?.name || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const agentName = resBody?.data?.name || resBody?.data?.email || 'deleted agent';
      return isSuccess
...
// Agent Logout
{
  ...
  config: {
    ...
    entity: ActivityEntity.AGENT,
    getEntityId: (req, resBody) => (req as AgentRequest).agent?.id || 'unknown',
    getEntityName: (req, resBody) => (req as AgentRequest).agent?.agentEmail || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
...
// Get Calendar Availability
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || 'unknown';
      const startDate = req.body?.startDate || '';
      const endDate = req.body?.endDate || '';
      const dateRange = startDate && endDate ? ` for ${startDate} to ${endDate}` : '';
      return isSuccess
...
// Create New Inventory
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.INVENTORY,
    getEntityId: (req, resBody) => resBody?.data?.inventoryId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.roomTypeName || req.body?.roomTypeName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const roomTypeName = resBody?.data?.roomTypeName || req.body?.roomTypeName || 'unknown';
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || '';
      return isSuccess
...
// Map Rate Plans to Inventory
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      ...
      const ratePlanCount = req.body?.ratePlans?.length || req.body?.ratePlanIds?.length || 0;
      return isSuccess
...
// Update Rate Plan Price
{
  ...
  config: {
    ...
    getEntityId: (req, resBody) => req.body?.ratePlanId || resBody?.data?.ratePlanId || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.ratePlanName || req.body?.ratePlanName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const ratePlanName = resBody?.data?.ratePlanName || req.body?.ratePlanName || 'rate plan';
...
// Update or Create Rate Plan Charges
{
  ...
  config: {
    ...
    getEntityId: (req, resBody) => resBody?.data?.chargeId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.chargeName || req.body?.chargeName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
...
},// Create Rate Plan
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.RATE_PLAN,
    getEntityId: (req, resBody) => resBody?.data?.ratePlanId || resBody?.data?._id || resBody?.data?.ratePlanCode || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.ratePlanName || req.body?.ratePlanName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const ratePlanName = resBody?.data?.name || req.body?.name || resBody?.data?.ratePlanName || req.body?.ratePlanName || 'unknown';
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || '';
      return isSuccess
...
// Update Rate Plan
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.RATE_PLAN,
    getEntityId: (req, resBody) => req.params?.ratePlanCode || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.ratePlanName || req.body?.ratePlanName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const ratePlanName = resBody?.data?.name || req.body?.name || resBody?.data?.ratePlanName || req.body?.ratePlanName || req.params?.ratePlanCode || 'unknown';
      return isSuccess
...
// Delete Rate Plan
{
  ...
  config: {
    action: ActivityAction.DELETE,
    entity: ActivityEntity.RATE_PLAN,
    getEntityId: (req, resBody) => req.params?.ratePlanCode || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || resBody?.data?.ratePlanName || req.params?.ratePlanCode || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const ratePlanName = resBody?.data?.name || resBody?.data?.ratePlanName || req.params?.ratePlanCode || 'deleted rate plan';
      return isSuccess
...
// Add Tax Group to Rate Plan
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.RATE_PLAN,
    getEntityId: (req, resBody) => req.body?.ratePlanId || req.body?.ratePlanCode || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.ratePlanName || req.body?.ratePlanName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const ratePlanName = resBody?.data?.ratePlanName || req.body?.ratePlanName || 'rate plan';
      const taxGroupName = req.body?.taxGroupName || req.body?.taxGroupId || '';
      return isSuccess
...
// Apply Restrictions (CTA/CTD)
{
  ...
  config: {
    ...
    getEntityId: (req, resBody) => req.body?.propertyCode || resBody?.data?.propertyCode || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.propertyName || req.body?.propertyName || req.body?.propertyCode || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || req.body?.propertyCode || 'property';
      const restrictionType = req.body?.restrictionType || '';
      const dateRange = req.body?.startDate && req.body?.endDate ? ` for ${req.body.startDate} to ${req.body.endDate}` : '';
      return isSuccess
...
// Add Addon to Rate Plan
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const ratePlanName = resBody?.data?.ratePlanName || req.body?.ratePlanName || 'rate plan';
      const addonName = req.body?.addonName || req.body?.addonId || '';
      return isSuccess
...
// Get Room Rent Price
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      ...
      const dateRange = checkIn && checkOut ? ` for ${checkIn} to ${checkOut}` : '';
      return isSuccess
...
// Create/Update Start Stop Sell
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || 'property';
      const action = req.body?.isSellable === false || req.body?.action === 'stop' ? 'stopped' : 'started';
      const dateRange = req.body?.startDate && req.body?.endDate ? ` for ${req.body.startDate} to ${req.body.endDate}` : '';
      return isSuccess
...
// Create Creation
{
  ...
  config: {
    ...
    getEntityId: (req, resBody) => resBody?.data?.creationId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.propertyName || req.body?.propertyName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const creationName = resBody?.data?.name || req.body?.name || resBody?.data?.propertyName || req.body?.propertyName || 'unknown';
      return isSuccess
...
// Toggle Draft Status
{
  ...
  config: {
    ...
    entity: ActivityEntity.CREATION,
    getEntityId: (req, resBody) => req.params?.id || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || resBody?.data?.propertyName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const creationName = resBody?.data?.name || resBody?.data?.propertyName || 'creation';
      const isDraft = resBody?.data?.isDraft ?? req.body?.isDraft;
      const status = isDraft === false ? 'published' : isDraft === true ? 'drafted' : 'toggled';
      return isSuccess
...
// Reset Password
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.USER,
    getEntityId: (req, resBody) => resBody?.data?.userId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => req.body?.email || resBody?.data?.email || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const email = req.body?.email || resBody?.data?.email || 'unknown';
      return isSuccess
...
// Assign User to Property
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.USER,
    getEntityId: (req, resBody) => req.body?.userId || resBody?.data?.userId || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.userName || req.body?.userName || resBody?.data?.email || req.body?.email || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const userName = resBody?.data?.userName || req.body?.userName || resBody?.data?.email || req.body?.email || 'user';
      const propertyName = req.body?.propertyName || req.body?.propertyId || '';
      return isSuccess
...
// Update User
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const userName = resBody?.data?.userName || req.body?.userName || resBody?.data?.email || req.body?.email || 'unknown';
      return isSuccess
...
// Delete User
{
  ...
  config: {
    ...
    entity: ActivityEntity.USER,
    getEntityId: (req, resBody) => req.params?.id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.userName || resBody?.data?.email || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const userName = resBody?.data?.userName || resBody?.data?.email || 'deleted user';
      return isSuccess
...
// Fetch Rooms for Booking
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      ...
      const checkOut = req.body?.checkOut || req.body?.checkOutDate || '';
      const dateRange = checkIn && checkOut ? ` for ${checkIn} to ${checkOut}` : '';
      const roomCount = resBody?.data?.rooms?.length || resBody?.data?.availableRooms?.length || 0;
      return isSuccess
...
// Create Creation Loyalty
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.LOYALTY_CONFIG,
    getEntityId: (req, resBody) => resBody?.data?.creationLoyalityId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.programName || req.body?.programName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const loyaltyName = resBody?.data?.name || req.body?.name || resBody?.data?.programName || req.body?.programName || 'unknown';
      const creationName = resBody?.data?.creationName || req.body?.creationName || '';
      return isSuccess
...
// Update Creation Loyalty
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.LOYALTY_CONFIG,
    getEntityId: (req, resBody) => req.params?.creationLoyalityId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.programName || req.body?.programName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const loyaltyName = resBody?.data?.name || req.body?.name || resBody?.data?.programName || req.body?.programName || 'unknown';
...
// Delete Creation Loyalty
{
  ...
  config: {
    action: ActivityAction.DELETE,
    entity: ActivityEntity.LOYALTY_CONFIG,
    getEntityId: (req, resBody) => req.params?.creationLoyalityId || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || resBody?.data?.programName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const loyaltyName = resBody?.data?.name || resBody?.data?.programName || 'deleted loyalty program';
      return isSuccess
...
// Create Loyalty Condition
{
  ...
  config: {
    ...
    getEntityId: (req, resBody) => resBody?.data?.conditionId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.conditionName || req.body?.conditionName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const conditionName = resBody?.data?.name || req.body?.name || resBody?.data?.conditionName || req.body?.conditionName || 'unknown';
      const programName = resBody?.data?.programName || req.body?.programName || '';
      return isSuccess
...
// Delete Loyalty Condition
{
  ...
  config: {
    ...
    entity: ActivityEntity.LOYALTY_CONDITION,
    getEntityId: (req, resBody) => req.params?.id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || resBody?.data?.conditionName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const conditionName = resBody?.data?.name || resBody?.data?.conditionName || 'deleted loyalty condition';
      return isSuccess
...
// Create Special Loyalty Condition
{
  ...
  config: {
    ...
    tags: ['loyalty', 'loyalty-condition', 'special-condition', 'create']
  }
},
...
// Update Special Loyalty Condition
{
  ...
  config: {
    ...
    tags: ['loyalty', 'loyalty-condition', 'special-condition', 'update']
  }
},
...
// Delete Special Loyalty Condition
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const conditionName = resBody?.data?.name || resBody?.data?.conditionName || 'deleted special loyalty condition';
      return isSuccess
        ? `Special loyalty condition '${conditionName}' deleted successfully`
        : `Failed to delete special loyalty condition`;
    },
    tags: ['loyalty', 'loyalty-condition', 'special-condition', 'delete']
  }
},
// Create Loyalty Field
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.LOYALTY_CONFIG,
    getEntityId: (req, resBody) => resBody?.data?.fieldId || resBody?.data?._id || req.body?.loyaltyProgramId || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.fieldName || req.body?.fieldName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const fieldName = resBody?.data?.fieldName || req.body?.fieldName || 'unknown';
      const programName = resBody?.data?.programName || req.body?.programName || '';
      return isSuccess
...
// Update Many Loyalty Fields
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.LOYALTY_CONFIG,
    getEntityId: (req, resBody) => req.params?.loyaltyProgramId || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.programName || req.body?.programName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const programName = resBody?.data?.programName || req.body?.programName || 'loyalty program';
      const fieldCount = req.body?.fields?.length || resBody?.data?.fieldsUpdated || 0;
      return isSuccess
    ...
    tags: ['loyalty', 'loyalty-field', 'update', 'bulk-update', 'configuration']
  }
},
...
// Update Single Loyalty Field
{
  ...
  config: {
    ...
    entity: ActivityEntity.LOYALTY_CONFIG,
    getEntityId: (req, resBody) => req.params?.loyaltyProgramId || 'unknown',
    getEntityName: (req, resBody) => req.params?.fieldName || resBody?.data?.fieldName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const fieldName = req.params?.fieldName || resBody?.data?.fieldName || 'unknown';
      const programName = resBody?.data?.programName || req.body?.programName || '';
      return isSuccess
...
// Delete Loyalty Field
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const fieldName = req.params?.fieldName || resBody?.data?.fieldName || 'deleted field';
...
// Delete Loyalty Guest
{
  ...
  config: {
    ...
    entity: ActivityEntity.LOYALTY_GUEST,
    getEntityId: (req, resBody) => req.params?.id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.email || resBody?.data?.guestName || resBody?.data?.name || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const guestName = resBody?.data?.guestName || resBody?.data?.name || resBody?.data?.email || 'deleted loyalty guest';
      return isSuccess
...
// Register Guest from Booking Engine
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.LOYALTY_GUEST,
    getEntityId: (req, resBody) => resBody?.data?.loyaltyGuestId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.email || req.body?.email || resBody?.data?.guestName || req.body?.guestName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const guestName = resBody?.data?.guestName || req.body?.guestName || resBody?.data?.email || req.body?.email || 'unknown';
      const programName = resBody?.data?.programName || req.body?.programName || '';
      return isSuccess
        ? `Loyalty guest '${guestName}' registered${programName ? ` to program '${programName}'` : ''}`
        : `Failed to register loyalty guest '${guestName}'`;
    },
    tags: ['loyalty', 'loyalty-guest', 'registration', 'create', 'booking-engine']
  }
},
// Create Loyalty Program
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.LOYALTY_CONFIG,
    getEntityId: (req, resBody) => resBody?.data?.loyaltyProgramId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.programName || req.body?.programName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const programName = resBody?.data?.name || req.body?.name || resBody?.data?.programName || req.body?.programName || 'unknown';
      const creationName = resBody?.data?.creationName || req.body?.creationName || '';
      return isSuccess
...
// Update Loyalty Program
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.LOYALTY_CONFIG,
    getEntityId: (req, resBody) => req.params?.loyaltyProgramId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.programName || req.body?.programName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const programName = resBody?.data?.name || req.body?.name || resBody?.data?.programName || req.body?.programName || 'unknown';
...
// Delete Loyalty Program
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const programName = resBody?.data?.name || resBody?.data?.programName || 'deleted loyalty program';
      return isSuccess
...
// Delete Advance Loyalty Program
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const programName = resBody?.data?.name || resBody?.data?.programName || 'deleted advance loyalty program';
      return isSuccess
...
// Create Property Loyalty Config
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.LOYALTY_CONFIG,
    getEntityId: (req, resBody) => resBody?.data?.propertyLoyalityId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.propertyName || req.body?.propertyName || resBody?.data?.programName || req.body?.programName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || 'property';
      const programName = resBody?.data?.programName || req.body?.programName || '';
...
// Update Property Loyalty Config
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.LOYALTY_CONFIG,
    getEntityId: (req, resBody) => req.params?.propertyId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.propertyName || req.body?.propertyName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || 'property';
...
// Delete Property Loyalty Config
{
  ...
  config: {
    ...
    entity: ActivityEntity.LOYALTY_CONFIG,
    getEntityId: (req, resBody) => req.params?.propertyId || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.propertyName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
...
// Create Reservation
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.RESERVATION,
    getEntityId: (req, resBody) => resBody?.data?.reservationId || resBody?.data?._id || resBody?.data?.reservationCode || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.guestName || req.body?.data?.guestDetails?.guestName || resBody?.data?.reservationCode || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const guestName = resBody?.data?.guestName || req.body?.data?.guestDetails?.guestName || 'guest';
      const reservationCode = resBody?.data?.reservationCode || resBody?.data?.bookingCode || '';
      const propertyName = resBody?.data?.propertyName || req.body?.data?.bookingDetails?.propertyName || '';
      return isSuccess
...
// Cancel Reservation
{
  ...
  config: {
    ...
    entity: ActivityEntity.RESERVATION,
    getEntityId: (req, resBody) => req.params?.reservationId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.reservationCode || resBody?.data?.guestName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const reservationCode = resBody?.data?.reservationCode || resBody?.data?.bookingCode || 'reservation';
...
// Update Reservation
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.RESERVATION,
    getEntityId: (req, resBody) => req.params?.reservationCode || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => req.params?.reservationCode || resBody?.data?.reservationCode || resBody?.data?.guestName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const reservationCode = req.params?.reservationCode || resBody?.data?.reservationCode || 'reservation';
      const guestName = resBody?.data?.guestName || '';
      return isSuccess
...
// Create Policy
{
  ...
  config: {
    ...
    getEntityId: (req, resBody) => resBody?.data?.policyId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.policyName || req.body?.policyName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const policyName = resBody?.data?.name || req.body?.name || resBody?.data?.policyName || req.body?.policyName || 'unknown';
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || '';
      return isSuccess
...
// Delete Policy
{
  ...
  config: {
    ...
    entity: ActivityEntity.POLICY,
    getEntityId: (req, resBody) => req.params?.id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || resBody?.data?.policyName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const policyName = resBody?.data?.name || resBody?.data?.policyName || 'deleted policy';
      return isSuccess
...
// Add Policy to Rate Plan
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.POLICY,
    getEntityId: (req, resBody) => req.body?.policyId || resBody?.data?.policyId || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.policyName || req.body?.policyName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const policyName = resBody?.data?.policyName || req.body?.policyName || 'policy';
      const ratePlanName = req.body?.ratePlanName || req.body?.ratePlanCode || '';
      return isSuccess
...
// Create Promo Code
{
  ...
  config: {
    ...
    getEntityId: (req, resBody) => resBody?.data?.promoCodeId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.code || req.body?.promoCodeData?.code || resBody?.data?.promoCode || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const promoCode = resBody?.data?.code || req.body?.promoCodeData?.code || resBody?.data?.promoCode || 'unknown';
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || '';
      const discountValue = req.body?.promoCodeData?.discountValue || resBody?.data?.discountValue || '';
      return isSuccess
...
// Update Promo Code
{
  ...
  config: {
    ...
    entity: ActivityEntity.PROMO_CODE,
    getEntityId: (req, resBody) => req.params?.id || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.code || req.body?.code || resBody?.data?.promoCode || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const promoCode = resBody?.data?.code || req.body?.code || resBody?.data?.promoCode || 'unknown';
      return isSuccess
...
// Delete Promo Code
{
  ...
  config: {
    ...
    entity: ActivityEntity.PROMO_CODE,
    getEntityId: (req, resBody) => req.params?.id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.code || resBody?.data?.promoCode || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const promoCode = resBody?.data?.code || resBody?.data?.promoCode || 'deleted promo code';
      return isSuccess
...
// Recover Promo Code
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const promoCode = resBody?.data?.code || resBody?.data?.promoCode || 'promo code';
      return isSuccess
...
// Create Customizable Deal
{
  ...
  config: {
    ...
    getEntityId: (req, resBody) => resBody?.data?.dealId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.dealName || req.body?.dealName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const dealName = resBody?.data?.name || req.body?.name || resBody?.data?.dealName || req.body?.dealName || 'unknown';
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || '';
      return isSuccess
...
// Update Customizable Deal
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.PROMOTION,
    getEntityId: (req, resBody) => req.params?.dealId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.dealName || req.body?.dealName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const dealName = resBody?.data?.name || req.body?.name || resBody?.data?.dealName || req.body?.dealName || 'unknown';
...
// Delete Customizable Deal
{
  ...
  config: {
    ...
    getEntityName: (req, resBody) => resBody?.data?.name || resBody?.data?.dealName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const dealName = resBody?.data?.name || resBody?.data?.dealName || 'deleted customizable deal';
      return isSuccess
...
// Create Device-Specific Promotion
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.PROMOTION,
    getEntityId: (req, resBody) => resBody?.data?.promotionId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.promotionName || req.body?.promotionName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const promotionName = resBody?.data?.name || req.body?.name || resBody?.data?.promotionName || req.body?.promotionName || 'unknown';
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || '';
      const deviceType = req.body?.deviceType || resBody?.data?.deviceType || '';
      return isSuccess
...
// Update Device-Specific Promotion
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.PROMOTION,
    getEntityId: (req, resBody) => req.params?.promotionId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.promotionName || req.body?.promotionName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const promotionName = resBody?.data?.name || req.body?.name || resBody?.data?.promotionName || req.body?.promotionName || 'unknown';
...
// Delete Device-Specific Promotion
{
  ...
  config: {
    ...
    getEntityName: (req, resBody) => resBody?.data?.name || resBody?.data?.promotionName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const promotionName = resBody?.data?.name || resBody?.data?.promotionName || 'deleted device-specific promotion';
      return isSuccess
...
// Toggle Device-Specific Promotion Status
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const promotionName = resBody?.data?.name || resBody?.data?.promotionName || 'promotion';
      const status = resBody?.data?.isActive ?? req.body?.isActive;
      const statusText = status === true ? 'activated' : status === false ? 'deactivated' : 'toggled';
      return isSuccess
    ...
    tags: ['promotion', 'device-specific', 'status', 'update', 'toggle']
  }
},
// Create Early-Bird Promotion
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      ...
      const promotionName = resBody?.data?.name || req.body?.name || resBody?.data?.promotionName || req.body?.promotionName || 'unknown';
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || '';
      const advanceDays = req.body?.advanceBookingDays || resBody?.data?.advanceBookingDays || '';
      return isSuccess
...
// Delete Early-Bird Promotion
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const promotionName = resBody?.data?.name || resBody?.data?.promotionName || 'deleted early-bird promotion';
      return isSuccess
...
// Create Offer-For-Tonight Promotion
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      ...
      const promotionName = resBody?.data?.name || req.body?.name || resBody?.data?.promotionName || req.body?.promotionName || 'unknown';
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || '';
      const discountValue = req.body?.discountValue || resBody?.data?.discountValue || '';
      return isSuccess
...
// Delete Offer-For-Tonight Promotion
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const promotionName = resBody?.data?.name || resBody?.data?.promotionName || 'deleted offer-for-tonight promotion';
      return isSuccess
...
{
  pattern: /\/api\/v1\/promotion\/offer-for-tonight\/[^/]+\/toggle-status$/,
  method: 'PATCH',
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.PROMOTION,
    ...
    tags: ['promotion', 'offer-for-tonight', 'status', 'update', 'toggle', 'last-minute']
  }
},
// Create Geo Rate Plan
{
  ...
  config: {
    ...
    getEntityId: (req, resBody) => resBody?.data?.geoRatePlanId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.planName || req.body?.planName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const planName = resBody?.data?.name || req.body?.name || resBody?.data?.planName || req.body?.planName || 'unknown';
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || '';
      const countries = req.body?.countries || resBody?.data?.countries || [];
      const countryInfo = countries.length > 0 ? ` for ${countries.length} ${countries.length === 1 ? 'country' : 'countries'}` : '';
      return isSuccess
...
// Delete Geo Rate Plan
{
  ...
  config: {
    ...
    entity: ActivityEntity.GEO_RATE_PLAN,
    getEntityId: (req, resBody) => req.params?.id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || resBody?.data?.planName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const planName = resBody?.data?.name || resBody?.data?.planName || 'deleted geo rate plan';
      return isSuccess
...
// Create MLOS Rate Plan Rule
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.RATE_PLAN_RULE,
    getEntityId: (req, resBody) => resBody?.data?.ruleId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.ruleName || req.body?.ruleName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const ruleName = resBody?.data?.name || req.body?.name || resBody?.data?.ruleName || req.body?.ruleName || 'MLOS rule';
      const ratePlanName = req.body?.ratePlanName || resBody?.data?.ratePlanName || '';
    ...
    tags: ['promotion', 'mlos', 'rate-plan-rule', 'create', 'restrictions']
  }
},
...
// Update MLOS Rate Plan Rule
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.RATE_PLAN_RULE,
    getEntityId: (req, resBody) => req.params?.ratePlanId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.ruleName || req.body?.ruleName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const ruleName = resBody?.data?.name || req.body?.name || resBody?.data?.ruleName || req.body?.ruleName || 'MLOS rule';
    ...
    tags: ['promotion', 'mlos', 'rate-plan-rule', 'update', 'restrictions']
  }
},
...
// Delete MLOS Rate Plan Rule
{
  ...
  config: {
    ...
    getEntityName: (req, resBody) => resBody?.data?.name || resBody?.data?.ruleName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const ruleName = resBody?.data?.name || resBody?.data?.ruleName || 'deleted MLOS rule';
      return isSuccess
        ? `MLOS rule '${ruleName}' deleted successfully`
        : `Failed to delete MLOS rule`;
    },
    tags: ['promotion', 'mlos', 'rate-plan-rule', 'delete', 'restrictions']
  }
},
// Create Booking Engine Config
{
  ...
  config: {
    ...
    getEntityId: (req, resBody) => req.params?.id || resBody?.data?.propertyId || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.propertyName || req.body?.propertyName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || 'property';
    ...
    tags: ['booking-engine', 'configuration', 'create', 'property-management']
  }
},
...
// Update Booking Engine Config
{
  ...
  config: {
    ...
    tags: ['booking-engine', 'configuration', 'update', 'property-management']
  }
},
...
// Delete Booking Engine Config
{
  ...
  config: {
    ...
    tags: ['booking-engine', 'configuration', 'delete', 'property-management']
  }
},// Create Category
{
  ...
  config: {
    ...
    getEntityId: (req, resBody) => resBody?.data?.categoryId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.categoryName || req.body?.categoryName || resBody?.data?.name || req.body?.name || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const categoryName = resBody?.data?.categoryName || req.body?.categoryName || resBody?.data?.name || req.body?.name || 'unknown';
      return isSuccess
...
// Delete Category
{
  ...
  config: {
    action: ActivityAction.DELETE,
    entity: ActivityEntity.MASTER_CATEGORY,
    getEntityId: (req, resBody) => req.params?.categoryName || 'unknown',
    getEntityName: (req, resBody) => req.params?.categoryName || resBody?.data?.categoryName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const categoryName = req.params?.categoryName || resBody?.data?.categoryName || 'deleted category';
      return isSuccess
...
// Create Property Type
{
  ...
  config: {
    ...
    getEntityId: (req, resBody) => resBody?.data?.typeId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.propertyTypeName || req.body?.propertyTypeName || resBody?.data?.name || req.body?.name || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const typeName = resBody?.data?.propertyTypeName || req.body?.propertyTypeName || resBody?.data?.name || req.body?.name || 'unknown';
      return isSuccess
...
// Delete Property Type
{
  ...
  config: {
    action: ActivityAction.DELETE,
    entity: ActivityEntity.MASTER_TYPE,
    getEntityId: (req, resBody) => req.params?.propertyTypeName || 'unknown',
    getEntityName: (req, resBody) => req.params?.propertyTypeName || resBody?.data?.propertyTypeName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const typeName = req.params?.propertyTypeName || resBody?.data?.propertyTypeName || 'deleted property type';
      return isSuccess
...
// Create Amenity
{
  ...
  config: {
    ...
    getEntityId: (req, resBody) => resBody?.data?.amenityId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.amenityName || req.body?.amenityName || resBody?.data?.name || req.body?.name || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const amenityName = resBody?.data?.amenityName || req.body?.amenityName || resBody?.data?.name || req.body?.name || 'unknown';
      return isSuccess
...
// Update/Delete Amenities
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.MASTER_AMENITY,
    getEntityId: (req, resBody) => req.body?.amenityId || resBody?.data?.amenityId || 'unknown',
    getEntityName: (req, resBody) => req.body?.amenityName || resBody?.data?.amenityName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const amenityName = req.body?.amenityName || resBody?.data?.amenityName || 'amenities';
      return isSuccess
...
// Create Room Amenity
{
  ...
  config: {
    ...
    tags: ['management', 'amenity', 'room-amenity', 'master-data', 'create']
  }
},
...
// Update/Delete Room Amenities
{
  ...
  config: {
    ...
    tags: ['management', 'amenity', 'room-amenity', 'master-data', 'update']
  }
},
...
// Create Loyalty Guest Field
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.LOYALTY_CONFIG,
    getEntityId: (req, resBody) => resBody?.data?.fieldId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.fieldName || req.body?.fieldName || resBody?.data?.name || req.body?.name || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const fieldName = resBody?.data?.fieldName || req.body?.fieldName || resBody?.data?.name || req.body?.name || 'unknown';
      return isSuccess
        ? `Loyalty guest field '${fieldName}' created successfully`
        : `Failed to create loyalty guest field '${fieldName}'`;
    },
    tags: ['management', 'loyalty', 'guest-field', 'master-data', 'create']
  }
},
...
// Delete Loyalty Guest Field
{
  ...
  config: {
    ...
    entity: ActivityEntity.LOYALTY_CONFIG,
    getEntityId: (req, resBody) => req.params?.id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.fieldName || resBody?.data?.name || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const fieldName = resBody?.data?.fieldName || resBody?.data?.name || 'deleted loyalty guest field';
      return isSuccess
        ? `Loyalty guest field '${fieldName}' deleted successfully`
        : `Failed to delete loyalty guest field`;
    },
    tags: ['management', 'loyalty', 'guest-field', 'master-data', 'delete']
  }
},
// Add Bank Details
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.PROPERTY,
    getEntityId: (req, resBody) => resBody?.data?.propertyId || req.body?.propertyId || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.propertyName || req.body?.propertyName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || 'property';
      const bankName = req.body?.bankName || resBody?.data?.bankName || '';
      return isSuccess
...
// Update Payment Methods
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || 'property';
      const paymentMethods = req.body?.paymentMethods || resBody?.data?.paymentMethods || [];
      const methodCount = Array.isArray(paymentMethods) ? paymentMethods.length : 0;
      return isSuccess
    ...
    tags: ['payment', 'payment-methods', 'update', 'property-management']
  }
},
// Update Property Config
{
  ...
  config: {
    ...
    tags: ['property', 'configuration', 'update', 'property-management']
  }
},
// Create Property
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.PROPERTY,
    getEntityId: (req, resBody) => resBody?.data?.propertyId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.propertyName || req.body?.propertyName || resBody?.data?.name || req.body?.name || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || resBody?.data?.name || req.body?.name || 'unknown';
      const propertyCode = resBody?.data?.propertyCode || req.body?.propertyCode || '';
      return isSuccess
...
// Delete Property
{
  ...
  config: {
    ...
    entity: ActivityEntity.PROPERTY,
    getEntityId: (req, resBody) => req.params?.id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.propertyName || resBody?.data?.name || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const propertyName = resBody?.data?.propertyName || resBody?.data?.name || 'deleted property';
      return isSuccess
...
// Create Property Address
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      ...
      const country = req.body?.country || resBody?.data?.country || '';
      const location = city && country ? ` (${city}, ${country})` : city ? ` (${city})` : country ? ` (${country})` : '';
      return isSuccess
...
// Create Property Amenity
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || 'property';
      const amenities = req.body?.amenities || resBody?.data?.amenities || [];
...
// Create Room
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.ROOM,
    getEntityId: (req, resBody) => resBody?.data?.roomId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.roomName || req.body?.roomName || resBody?.data?.name || req.body?.name || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const roomName = resBody?.data?.roomName || req.body?.roomName || resBody?.data?.name || req.body?.name || 'unknown';
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || '';
      const roomType = req.body?.roomType || resBody?.data?.roomType || '';
      return isSuccess
...
// Update Room
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.ROOM,
    getEntityId: (req, resBody) => req.params?.roomId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.roomName || req.body?.roomName || resBody?.data?.name || req.body?.name || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const roomName = resBody?.data?.roomName || req.body?.roomName || resBody?.data?.name || req.body?.name || 'unknown';
...
// Delete Room
{
  ...
  config: {
    ...
    getEntityName: (req, resBody) => resBody?.data?.roomName || resBody?.data?.name || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const roomName = resBody?.data?.roomName || resBody?.data?.name || 'deleted room';
      return isSuccess
...
// Add 360 Image to Room
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const roomName = resBody?.data?.roomName || req.body?.roomName || resBody?.data?.name || req.body?.name || 'room';
      return isSuccess
...
// Create Room Amenity
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.ROOM,
    getEntityId: (req, resBody) => req.params?.roomId || resBody?.data?.roomId || 'unknown',
...
// Delete Room Amenity
{
  ...
  config: {
    ...
    entity: ActivityEntity.ROOM,
    getEntityId: (req, resBody) => req.params?.roomId || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.roomName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
...
// Create Property Video
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.PROPERTY,
    getEntityId: (req, resBody) => req.params?.propertyId || resBody?.data?.propertyId || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.propertyName || req.body?.propertyName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || 'property';
      const videoUrl = req.body?.videoUrl || resBody?.data?.videoUrl || '';
      return isSuccess
...
// Create Tax Group
{
  ...
  config: {
    ...
    getEntityId: (req, resBody) => resBody?.data?.taxGroupId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.groupName || req.body?.groupName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const groupName = resBody?.data?.name || req.body?.name || resBody?.data?.groupName || req.body?.groupName || 'unknown';
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || '';
      return isSuccess
...
// Update Tax Group
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.TAX_GROUP,
    getEntityId: (req, resBody) => req.params?.taxGroupId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.groupName || req.body?.groupName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const groupName = resBody?.data?.name || req.body?.name || resBody?.data?.groupName || req.body?.groupName || 'unknown';
...
// Delete Tax Group
{
  ...
  config: {
    ...
    getEntityName: (req, resBody) => resBody?.data?.name || resBody?.data?.groupName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const groupName = resBody?.data?.name || resBody?.data?.groupName || 'deleted tax group';
      return isSuccess
...
// Add Rules to Tax Group
{
  ...
  config: {
    ...
    entity: ActivityEntity.TAX_GROUP,
    getEntityId: (req, resBody) => req.params?.taxGroupId || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.groupName || req.body?.groupName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const groupName = resBody?.data?.groupName || req.body?.groupName || 'tax group';
...
// Create Tax Rule
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.TAX_RULE,
    getEntityId: (req, resBody) => resBody?.data?.taxRuleId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.ruleName || req.body?.ruleName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const ruleName = resBody?.data?.name || req.body?.name || resBody?.data?.ruleName || req.body?.ruleName || 'unknown';
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || '';
...
// Update Tax Rule
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.TAX_RULE,
    getEntityId: (req, resBody) => req.params?.taxRuleId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.ruleName || req.body?.ruleName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const ruleName = resBody?.data?.name || req.body?.name || resBody?.data?.ruleName || req.body?.ruleName || 'unknown';
...
// Delete Tax Rule
{
  ...
  config: {
    ...
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const ruleName = resBody?.data?.name || resBody?.data?.ruleName || 'deleted tax rule';
      return isSuccess
...
// Create Tourist Tax
{
  ...
  config: {
    action: ActivityAction.CREATE,
    entity: ActivityEntity.TAX_RULE,
    getEntityId: (req, resBody) => resBody?.data?.touristTaxId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.taxName || req.body?.taxName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const taxName = resBody?.data?.name || req.body?.name || resBody?.data?.taxName || req.body?.taxName || 'unknown';
      const propertyName = resBody?.data?.propertyName || req.body?.propertyName || '';
      const taxAmount = req.body?.amount || resBody?.data?.amount || '';
      return isSuccess
...
// Update Tourist Tax
{
  ...
  config: {
    action: ActivityAction.UPDATE,
    entity: ActivityEntity.TAX_RULE,
    getEntityId: (req, resBody) => req.params?.touristTaxId || resBody?.data?._id || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || req.body?.name || resBody?.data?.taxName || req.body?.taxName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const taxName = resBody?.data?.name || req.body?.name || resBody?.data?.taxName || req.body?.taxName || 'unknown';
...
// Delete Tourist Tax
{
  ...
  config: {
    action: ActivityAction.DELETE,
    entity: ActivityEntity.TAX_RULE,
    getEntityId: (req, resBody) => req.params?.touristTaxId || 'unknown',
    getEntityName: (req, resBody) => resBody?.data?.name || resBody?.data?.taxName || 'unknown',
    getDescription: (req, resBody, statusCode) => {
      const isSuccess = statusCode! >= 200 && statusCode! < 300;
      const taxName = resBody?.data?.name || resBody?.data?.taxName || 'deleted tourist tax';
      return isSuccess
