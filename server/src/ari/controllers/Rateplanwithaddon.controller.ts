...
export class RatePlanWithAddonController {
  ...
  public static async addAddonToRatePlan(req: CustomRequest, res: Response) {
    try {
      ...
      if (!ratePlanCode || !addonId) {
        return res
          .status(400)
          .json(errorResponse('Rate plan code and addon ID are required'));
      }
   ...
   * Body: { ratePlanCode: string, addonId: string }
   */
  public static async removeAddonFromRatePlan(req: CustomRequest, res: Response) {
    try {
      const { ratePlanCode, addonId } = req.body;
      ...
      const response = await RatePlanWithAddonService.removeAddonFromRatePlan(
        ratePlanCode,
        addonId
      );
   ...
   */
  public static async getAddonsByRatePlanCode(req: CustomRequest, res: Response) {
    try {
      ...
      const response = await RatePlanWithAddonService.getAddonsByRatePlanCode(
        ratePlanCode
      );
   ...
   */
  public static async getRatePlansByAddonId(req: CustomRequest, res: Response) {
    try {
      ...
      const response = await RatePlanWithAddonService.getRatePlansByAddonId(
        addonId
      );
