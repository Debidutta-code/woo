...
export class RatePlanServices {
    ...
    public static async deleteCharges(chargeId: string): Promise<IApiResponse> {
        try {
            const isExist = await RatePlanRepository.getChargesById(chargeId);
            if (!isExist) {
                return errorResponse('Charges does not exists');
            }
            const response = await RatePlanRepository.deleteCharges(chargeId);
            if (response) {
                return successResponse(
                    'Charges deleted successfully',
                    response
                );
            } else {
                return errorResponse(`Failed to delete charges`);
            }
        } catch (error: any) {
            return errorResponse(`Failed to delete charges`, error?.message);
        }
    }
}
