...
export class AvailabilityServices {
    public static async getCalendarAvailability(
        ...
        try {
            ...
            const [property, inventories, charges, reservations] =
                await Promise.all([
                    AvailabilityRepository.getPropertyByCode(
                        propertyCode,
                        roomTypeCodes,
                        ratePlanCodes
                    ),
                    AvailabilityRepository.getInventoryForDateRange(
                        propertyCode,
                        startDate,
                        endDate,
                        roomTypeCodes
                    ),
                    AvailabilityRepository.getChargesForDateRange(
                        propertyCode,
                        startDate,
                        endDate,
                        roomTypeCodes,
                    ...
                    AvailabilityRepository.getReservationsForDateRange(
                        propertyCode,
                        startDate,
                        endDate,
                        roomTypeCodes
            ...
            const filteredRooms =
                roomTypeCodes.length > 0
                    ? property.propertyRooms.filter(room =>
                          roomTypeCodes.includes(room.roomType)
                      )
                    : property.propertyRooms;

            // ✅ ADD: Filter rate plans if rate plan codes provided
            const filteredRatePlans =
                ratePlanCodes.length > 0
                    ? property.ratePlans.filter(rp =>
                          ratePlanCodes.includes(rp.ratePlanCode)
                      )
                    : property.ratePlans;
            ...
            const calendarDays = dates.map(date => {
                return this.buildDayData(
                    ...
                    {
                        ...property,
                        propertyRooms: filteredRooms,
                        ratePlans: filteredRatePlans,
                    }, // ✅ Pass filtered rate plans
                    inventories,
                    charges,
                    soldRoomsMap
                );
        ...
        dates.forEach(date => {
            ...
            reservations.forEach(reservation => {
                ...
                if (isWithin && !isSameCheckout) {
                    ...
                    const currentSold =
                        soldMap.get(dateKey)!.get(roomTypeCode) || 0;
    ...
    private static buildDayData(
        ...
        const roomTypes = property.propertyRooms.map((room: any) => {
            ...
            const hasCharges = dayCharges.some(
                c => c.roomTypeCode === room.roomType && !c.isSaleStopped
            );
            ...
            return {
                ...
                sold,
                occupancy:
                    totalInventory > 0 ? (sold / totalInventory) * 100 : 0,
        ...
        dayCharges.forEach(charge => {
            if (!ratePlanMap.has(charge.ratePlanCode)) {
                const ratePlan = property.ratePlans.find(
                    (rp: any) => rp.ratePlanCode === charge.ratePlanCode
                );
            ...
            const inventory = dayInventories.find(
                inv => inv.roomTypeCode === charge.roomTypeCode
            );
            ...
            ratePlanData.prices.push({
                ...
                sellStatus:
                    charge.isSaleStopped || available <= 0 ? 'close' : 'open',
                cta: charge.isClosedToArrival,
                ctd: charge.isClosedToDeparture,
                ...
                additionalGuestAmounts: charge.additionalGuestAmounts.map(
                    (ag: any) => ({
        ...
        const totalInventory = roomTypes.reduce(
            (sum: number, rt: any) => sum + rt._totalInventory,
            0
        );
        const totalSold = roomTypes.reduce(
            (sum: number, rt: any) => sum + rt.sold,
            0
        );
        const totalAvailable = roomTypes.reduce(
            (sum: number, rt: any) => sum + rt.available,
            0
        );
        ...
        return {
            ...
            occupancyPercent:
                totalInventory > 0 ? (totalSold / totalInventory) * 100 : 0,
            restrictions: {
