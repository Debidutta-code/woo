...
export class UserController {
    public static async getMe(req: CustomRequest, res: Response) {
        try {
            ...
            if (!userId) {
                return res
                    .status(400)
                    .json(errorResponse('User verification failed'));
            }
            const user = await AuthService.getUserById(userId);
            if (user?.success) {
                return res.status(200).json(successResponse('User data fetched successfully', {user:user.data,access:req?.permission}));
            } else {
        ...
        try {
            ...
            if (user?.success) {
                return res
                    .status(200)
                    .json(
                        successResponse('User data fetched successfully', user)
                    );
            } else {
    ...
    public static async getUserForMappingController(
        req: CustomRequest,
        res: Response
    ) {
        try {
        ...
        try {
            ...
            if (!creationId) {
                return res
                    ...
                    .json(
                        errorResponse(
                            `User is not mapped to any ${userRole === 'hotel_manager' ? 'Property' : userRole === 'group_manager' ? 'Group' : userRole === 'brand_manager' ? 'Brand' : 'Creation'}`
                        )
                    );
            }
            const users = await AuthService.getUserCreatedById(
                userId,
                userRole,
                creationId
            );
        ...
        try {
            ...
            if (
                ...
                return res
                    .status(403)
                    .json(
                        errorResponse('You did not access to delete this role')
                    );
            } else if (
    ...
    public static async mapUser(req: CustomRequest, res: Response) {
        try {
            ...
            const roleData = allRoles.data.find(
                (roleItem: any) => roleItem.role == role
            );
    ...
    // Forgot Password - Send OTP or Link
    ...
    public static async verifyResetOTP(req: CustomRequest, res: Response) {
        try {
            ...
            if (!email || !otp) {
                return res
                    .status(400)
                    .json(errorResponse('Email and OTP are required'));
            }
    ...
    // Verify reset token (for link-based password reset)
    public static async verifyResetToken(req: CustomRequest, res: Response) {
        try {
            ...
            if (!token) {
                return res
                    .status(400)
                    .json(errorResponse('Reset token is required'));
            }
    ...
    // Reset password with token (for link-based password reset)
    public static async resetPasswordWithToken(
        ...
        try {
            ...
            if (!token || !password) {
                return res
                    .status(400)
                    .json(errorResponse('Token and password are required'));
            }
