        ...
        try {
            ...
            if (!rates) {
                return res
                    ...
                    .json(
                        errorResponse(
                            'No exchange rates found. Please trigger a manual fetch.'
                        )
                    );
            }
    ...
    public getCurrencyRate = async (req: Request, res: Response) => {
        try {
            ...
            if (!currency || typeof currency !== 'string') {
                return res
                    .status(400)
                    .json(errorResponse('Currency code is required'));
            }
            ...
            if (rate === null) {
                return res
                    ...
                    .json(
                        errorResponse(
                            `Rate not found for currency: ${currency.toUpperCase()}`
                        )
                    );
            }
        ...
        try {
            ...
            if (!rates) {
                return res
                    ...
                    .json(
                        errorResponse(
                            'No exchange rates found in hash. Please trigger a manual fetch.'
                        )
                    );
            }
    ...
    public getMetadata = async (req: Request, res: Response) => {
        try {
            ...
            if (!metadata || Object.keys(metadata).length === 0) {
                return res.status(404).json(errorResponse('No metadata found'));
            }

            return res
                .status(200)
                .json(
                    successResponse('Metadata retrieved successfully', metadata)
                );
        } catch (error) {
