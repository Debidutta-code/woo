...
export class PropertyEmailController {
    ...
    public async createPropertyEmail(
        req: CustomRequest,
        res: Response
    ): Promise<Response> {
        try {
            ...
            if (error instanceof Error) {
                return res
                    ...
                    .json(
                        errorResponse(
                            'failed to create property email',
                            error.message
                        )
                    );
            }
            return res
                .status(500)
                .json(errorResponse('failed to create property email'));
        }
    }
    public async getPropertyEmails(
        ...
        try {
            ...
            const result =
                await this.propertyEmailsService.getPropertyEmails(propertyId);
            if (result.success) {
            ...
            return res
                .status(500)
                .json(errorResponse('failed to retrieve property emails'));
        }
    }
    public async deletePropertyEmail(
        req: CustomRequest,
        res: Response
    ): Promise<Response> {
        try {
            ...
            if (!id) {
                return res
                    .status(400)
                    .json(errorResponse('email id is required'));
            }
            const result =
                await this.propertyEmailsService.deletePropertyEmail(id);
            if (result.success) {
                return res.status(200).json(result);
            }
            return res.status(400).json(result);
            ...
            return res
                .status(500)
                .json(errorResponse('failed to delete property email'));
        }
    }
    public async updatePropertyEmail(
        ...
        try {
            ...
            return res
                .status(500)
                .json(errorResponse('failed to update property email'));
        }
    }
}
