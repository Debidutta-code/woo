...
export class PropertyConfigService {
    ...
    public async updatePropertyConfigService(
        propertyId: string,
        data: IUPropertyConfig
    ): Promise<IApiResponse> {
        try {
            const daoRes = await this.prppertyConfigRepo.updatePropertyConfig(
                propertyId,
                data
            );
            if (daoRes instanceof Error) {
                return errorResponse(
                    'Failed to update Property Config',
                    daoRes.message
                );
            }
    ...
    public async getPropertyConfig(propertyId: string): Promise<IApiResponse> {
        try {
            const daoRes =
                await this.prppertyConfigRepo.getConfigByProperty(propertyId);
            if (daoRes instanceof Error) {
                return errorResponse(
                    'Failed to fetch Property Config',
                    daoRes.message
                );
            }
            return successResponse(
                'property Config fetched successfully',
                daoRes
            );
        } catch (error) {
            if (error instanceof Error) {
