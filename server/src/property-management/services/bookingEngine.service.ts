...
export class BookingEngineService {
    public static async getConfigByPropertyId(propertyId: string) {
        try {
            const config =
                await BookingEngineConfigDao.getByPropertyId(propertyId);
            if (config) {
    ...
    public static async updateConfigByPropertyId(
        ...
        try {
            const updated =
                await BookingEngineConfigDao.updateConfigByPropertyId(
                    propertyId,
                    config
                );
            if (updated) {
    ...
    public static async deleteByPropertyId(propertyId: string) {
        try {
            const config =
                await BookingEngineConfigDao.deleteByPropertyId(propertyId);
            if (config) {
                return successResponse(
