...
export class PropertyEmailsService {
    ...
    public createPropertyEmail = async (propertyId: string, email: string): Promise<IApiResponse> => {
        try {
            const isAlreadyExists = await this.propertyEmailsRepository.getByEmail(propertyId, email);
            if(isAlreadyExists){
                return errorResponse("email already exists for this property");
            }
            const result = await this.propertyEmailsRepository.create(propertyId, email);
            if(!result){
            ...
            if(error instanceof Error){
                return errorResponse("failed to add email to property",error.message);
            }
            return errorResponse("failed to add email to property");
        }
    }
    public async getPropertyEmails(propertyId: string): Promise<IApiResponse> {
        try {
            const result = await this.propertyEmailsRepository.getByPropertyId(propertyId);
            return successResponse("Property emails retrieved successfully", result);
        } catch (error) {
            if(error instanceof Error){
                return errorResponse("failed to retrieve property emails",error.message);
            }
    ...
    public async deletePropertyEmail(id: string): Promise<IApiResponse> {
        try {
            const existingEmail = await this.propertyEmailsRepository.getById(id);
            if(!existingEmail){
            ...
            if(error instanceof Error){
                return errorResponse("failed to delete property email",error.message);
            }
            return errorResponse("failed to delete property email");
        }
    }
    public async updatePropertyEmail(id: string, email: string): Promise<IApiResponse> {
        try {
            const existingEmail = await this.propertyEmailsRepository.getById(id);
            if(!existingEmail){
                return errorResponse("email does not exist");
            }
            const isAlreadyExists = await this.propertyEmailsRepository.getByEmail(existingEmail.propertyId, email);
            if(isAlreadyExists){
                return errorResponse("email already exists for this property");
            }
            ...
            if(error instanceof Error){
                return errorResponse("failed to update property email",error.message);
            }
