...
export default function AccessControlPage() {
  ...
  const handleCreateRole = async () => {
    ...
    const role: IAccess = {
      ...
      canDeleteCTA: false,
      canViewCTA: false,
      canCreateGeoRatePlan: false,
      canUpdateGeoRatePlan: false,
      canDeleteGeoRatePlan: false,
      canViewGeoRatePlan: false,
                ...
                {roles.map((role) => (
                  <SelectItem key={role.role} value={role.role}>
                    <div className="flex items-center justify-between w-full">
                      <span className="font-medium">{role.role.replace(/_/g, " ").toLocaleUpperCase()}</span>
                    </div>
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <Card className="bg-card border border-border overflow-hidden">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <CardTitle className="text-xl font-bold text-foreground">
                      {selectedRole.role.replace(/_/g, " ").toLocaleUpperCase()}
                    </CardTitle>
            ...
            <CardContent className="pt-6 pb-8">
              ...
              <Section title="Inventory & Availability" icon={<Settings className="h-5 w-5" />}>
                ...
                />
                <PermissionToggle
                  label="View Inventory And Price"
              ...
              <Section title="Tax Management" icon={<Settings className="h-5 w-5" />}>
                <PermissionToggle
                  label="View Tax"
                  checked={selectedRole.canViewTax}
                  onChange={() => handlePermissionToggle('canViewTax')}
                />
                <PermissionToggle
                  label="Add Tax"
                  checked={selectedRole.canAddTax}
                  onChange={() => handlePermissionToggle('canAddTax')}
                />
                <PermissionToggle
                  label="Update Tax"
                  checked={selectedRole.canUpdateTax}
                  onChange={() => handlePermissionToggle('canUpdateTax')}
                />
                <PermissionToggle
                  label="Delete Tax"
                  checked={selectedRole.canDeleteTax}
                  onChange={() => handlePermissionToggle('canDeleteTax')}
                />
                <PermissionToggle
                  label="Create Tax Group"
                  checked={selectedRole.canCreateTaxGroup}
                  onChange={() => handlePermissionToggle('canCreateTaxGroup')}
                />
                <PermissionToggle
                  label="Delete Tax Group"
                  checked={selectedRole.canDeleteTaxGroup}
                  onChange={() => handlePermissionToggle('canDeleteTaxGroup')}
                />
                <PermissionToggle
                  label="Add Tax to Rate Plans"
                  checked={selectedRole.canAddTaxToRatePlans}
                  onChange={() => handlePermissionToggle('canAddTaxToRatePlans')}
                />
              </Section>
              {/* Booking */}
              ...
              {/* Start/Stop Sell */}
              <Section title="Start/Stop Sell" icon={<Settings className="h-5 w-5" />}>
                <PermissionToggle
                  label="Modify Start/Stop Sell"
                  checked={selectedRole.canModifyStartStopSell}
                  onChange={() => handlePermissionToggle('canModifyStartStopSell')}
                />
              </Section>

              {/* Room Types */}
              <Section title="Room Types Management" icon={<Settings className="h-5 w-5" />}>
                <PermissionToggle
                  label="Create Room Types"
                  checked={selectedRole.canCreateRoomTypes}
                  onChange={() => handlePermissionToggle('canCreateRoomTypes')}
                />
                <PermissionToggle
                  label="Update Room Types"
                  checked={selectedRole.canUpdateRoomTypes}
                  onChange={() => handlePermissionToggle('canUpdateRoomTypes')}
                />
                <PermissionToggle
                  label="Delete Room Types"
                  checked={selectedRole.canDeleteRoomTypes}
                  onChange={() => handlePermissionToggle('canDeleteRoomTypes')}
                />
              </Section>

              {/* Reservation */}
              <Section title="Reservation Management" icon={<Settings className="h-5 w-5" />}>
                <PermissionToggle
                  label="Create Reservation"
                  checked={selectedRole.canCreateReservation}
                  onChange={() => handlePermissionToggle('canCreateReservation')}
                />
                <PermissionToggle
                  label="View Reservation"
                  checked={selectedRole.canViewReservation}
                  onChange={() => handlePermissionToggle('canViewReservation')}
                />
                <PermissionToggle
                  label="Cancel Reservation"
                  checked={selectedRole.canCancelReservation}
                  onChange={() => handlePermissionToggle('canCancelReservation')}
                />
                <PermissionToggle
                  label="Amend Reservation"
                  checked={selectedRole.canAmendReservation}
                  onChange={() => handlePermissionToggle('canAmendReservation')}
                />
              </Section>

              {/* Addons */}
              <Section title="Addons Management" icon={<Settings className="h-5 w-5" />}>
                <PermissionToggle
                  label="View Addons"
                  checked={selectedRole.canViewAddons}
                  onChange={() => handlePermissionToggle('canViewAddons')}
                />
                <PermissionToggle
                  label="Add Addons"
                  checked={selectedRole.canAddAddons}
                  onChange={() => handlePermissionToggle('canAddAddons')}
                />
                <PermissionToggle
                  label="Update Addons"
                  checked={selectedRole.canUpdateAddons}
                  onChange={() => handlePermissionToggle('canUpdateAddons')}
                />
                <PermissionToggle
                  label="Delete Addons"
                  checked={selectedRole.canDeleteAddons}
                  onChange={() => handlePermissionToggle('canDeleteAddons')}
                />
                <PermissionToggle
                  label="Add Addons for Bookings"
                  checked={selectedRole.canAddAddonsForBookings}
                  onChange={() => handlePermissionToggle('canAddAddonsForBookings')}
                />
              </Section>
              ...
              <Section title="Policy to Rate Plans" icon={<Settings className="h-5 w-5" />}>
                <PermissionToggle
                  label="Add Policy to Rate Plans"
                  checked={selectedRole.canAddPolicyToRatePlans}
                  onChange={() => handlePermissionToggle('canAddPolicyToRatePlans')}
                />
              </Section>

              {/* 360 Images */}
              <Section title="360 Images" icon={<Settings className="h-5 w-5" />}>
                <PermissionToggle
                  label="Add 360 Images"
                  checked={selectedRole.canAdd360Images}
                  onChange={() => handlePermissionToggle('canAdd360Images')}
                />
              </Section>

              {/* CTA Management */}
              <Section title="CTA Management" icon={<Settings className="h-5 w-5" />}>
                <PermissionToggle
                  label="Create CTA"
                  checked={selectedRole.canCreateCTA}
                  onChange={() => handlePermissionToggle('canCreateCTA')}
                />
                <PermissionToggle
                  label="Update CTA"
                  checked={selectedRole.canUpdateCTA}
                  onChange={() => handlePermissionToggle('canUpdateCTA')}
                />
                <PermissionToggle
                  label="Delete CTA"
                  checked={selectedRole.canDeleteCTA}
                  onChange={() => handlePermissionToggle('canDeleteCTA')}
                />
                <PermissionToggle
                  label="View CTA"
                  checked={selectedRole.canViewCTA}
                  onChange={() => handlePermissionToggle('canViewCTA')}
                />
              </Section>

              {/* Geo Rate Plan Management */}
              <Section title="Geo Rate Plan Management" icon={<Settings className="h-5 w-5" />}>
                <PermissionToggle
                  label="Create Geo Rate Plan"
                  checked={selectedRole.canCreateGeoRatePlan}
                  onChange={() => handlePermissionToggle('canCreateGeoRatePlan')}
                />
                <PermissionToggle
                  label="Update Geo Rate Plan"
                  checked={selectedRole.canUpdateGeoRatePlan}
                  onChange={() => handlePermissionToggle('canUpdateGeoRatePlan')}
                />
                <PermissionToggle
                  label="Delete Geo Rate Plan"
                  checked={selectedRole.canDeleteGeoRatePlan}
                  onChange={() => handlePermissionToggle('canDeleteGeoRatePlan')}
                />
                <PermissionToggle
                  label="View Geo Rate Plan"
                  checked={selectedRole.canViewGeoRatePlan}
                  onChange={() => handlePermissionToggle('canViewGeoRatePlan')}
                />
              </Section>
            </CardContent>
          </Card>
